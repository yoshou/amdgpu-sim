...
fn main() -> Result<()> {
    ...
    if let Ok(elffile) = ElfFile::parse(&data) {
            ...
            .find(|sym| sym.name() == Some(kernel_name))
        {
            ...
            let group_segment_size = kernel_desc.group_segment_fixed_size;
            ...
            let aql = HsaKernelDispatchPacket {
                ...
                group_segment_size: group_segment_size as u32,
                kernel_object: Pointer::new(&mem, kernel_addr),
                kernarg_address: Pointer::new(&arg_buffer, 0),
            };

            let dims = GridDims {
                num_wg_x: total_blocks as u32,
                num_wg_y: 1,
                num_wg_z: 1,
                wg_x: threads_per_block as u32,
                wg_y: 1,
                wg_z: 1,
            };
