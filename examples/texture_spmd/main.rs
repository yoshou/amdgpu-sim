...
fn main() -> Result<()> {
    ...
    let mut file = File::open(program_filename)?;
    let mut data = vec![];
    ...
    if let Ok(elffile) = ElfFile::parse(&data) {
        ...
        let (kernarg_seg_size, private_segment_size) = if let Metadata::Yaml(metadata) = metadata {
            ...
            (kernarg_seg_size, private_seg_fixed_size + stack_size)
            ...
            if version.amdhsa_version[0] == 1 && version.amdhsa_version[1] == 2 {
                ...
                let is_dynamic_call_stack = map.amdhsa_kernels[0]
                    .uses_dynamic_stack
                    .unwrap_or(false);
                let stack_size = if is_dynamic_call_stack { 0x2000 } else { 0 };
                (kernarg_seg_size, private_seg_fixed_size + stack_size)
        ...
        println!("kernel_arg_ptr: 0x{:X}", (&arg_buffer[0] as *const u8) as u64);
