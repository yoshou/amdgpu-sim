...
const USE_STACK_CACHE: bool = false;
...
impl IREmitter {
    ...
    unsafe fn emit_u32x8_to_f64x8(&self, value: LLVMValueU32x8) -> LLVMValueF64x8 {
        let context = self.context;
        let builder = self.builder;
        let ty_i32 = llvm::core::LLVMInt32TypeInContext(context);
        let ty_i64 = llvm::core::LLVMInt64TypeInContext(context);
    ...
    unsafe fn emit_load_stack_sgpr_u32(&self, reg: u32) -> llvm::prelude::LLVMValueRef {
        let context = self.context;
        let builder = self.builder;
    ...
    unsafe fn emit_store_stack_sgpr_u32(&self, reg: u32, value: llvm::prelude::LLVMValueRef) {
        let builder = self.builder;
    ...
    unsafe fn emit_exp2_f64x8(&self, value0: LLVMValueF64x8) -> LLVMValueF64x8 {
        let context = self.context;
        let builder = self.builder;
        let empty_name = std::ffi::CString::new("").unwrap();
        let ty_f64 = llvm::core::LLVMDoubleTypeInContext(context);
...
impl RDNATranslator {
    ...
    fn analyze(&self) -> RegisterUsage {
        let mut reg_usage = RegisterUsage {
            use_sgprs: HashSet::new(),
            use_vgprs: HashSet::new(),
            def_sgprs: HashSet::new(),
            def_vgprs: HashSet::new(),
    ...
    unsafe fn emit_intrsuctions(
        ...
        for inst in &self.insts[..self.insts.len() - 1] {
            match inst {
                ...
                InstFormat::VOPC(inst) => match inst.op {
                    I::V_CMP_GT_U32 => {
                        bb = emitter.emit_vop_update_sgpr(bb, 106, |emitter, bb, elem| {
                            let empty_name = std::ffi::CString::new("").unwrap();
                    ...
                    I::V_CMPX_NGT_F64 => {
                        bb = emitter.emit_vop_update_sgpr(bb, 126, |emitter, bb, elem| {
                            let empty_name = std::ffi::CString::new("").unwrap();
                    ...
                    I::V_ADD_F64 => {
                        if USE_SIMD {
                            ...
                            for i in (0..32).step_by(8) {
                                ...
                                let exec_value = llvm::core::LLVMBuildLShr(
                                    builder,
                                    exec_value,
                                    elem,
                                    empty_name.as_ptr(),
                                ...
                                let exec_mask = llvm::core::LLVMBuildAnd(
                                    builder,
                                    exec_vec,
                                    bit_mask,
                                    empty_name.as_ptr(),
                ...
                InstFormat::VOP3(inst) => match inst.op {
                    ...
                    I::V_CMP_GT_U32 => {
                        bb = emitter.emit_vop_update_sgpr(
                            bb,
                            inst.vdst as u32,
                            |emitter, bb, elem| {
                                let empty_name = std::ffi::CString::new("").unwrap();
                    ...
                    I::V_FMA_F64 => {
                        if USE_SIMD {
                            ...
                            // bb = emitter.emit_vop_execmask(bb, |emitter, bb, elem, mask| {
                            //     let s0_value = emitter
                            //         .emit_vector_source_operand_f64x8(&inst.src0, elem, mask);
                    ...
                    I::V_LDEXP_F64 => {
                        if USE_SIMD {
                            ...
                            for i in (0..32).step_by(8) {
                                ...
                                let s1_value = emitter
                                    .emit_vector_source_operand_u32x8(&inst.src1, elem, mask);

                                let s0_value =
                                    emitter.emit_abs_neg_f64x8(s0_value, inst.abs, inst.neg, 0);
                                let s1_value = emitter.emit_u32x8_to_f64x8(s1_value);
                    ...
                    match inst.opx {
                        I::V_DUAL_CNDMASK_B32 => {
                            ...
                            for i in 0..32 {
                                ...
                                let s1_value = emitter.emit_load_vgpr_u32(inst.vsrc1x as u32, elem);
                    ...
                    match inst.opy {
                        I::V_DUAL_CNDMASK_B32 => {
                            ...
                            for i in 0..32 {
                                ...
                                let s1_value = emitter.emit_load_vgpr_u32(inst.vsrc1y as u32, elem);
