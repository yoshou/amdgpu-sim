...
pub fn f32_to_u32(val: f32) -> u32 {
    unsafe { std::mem::transmute::<f32, u32>(val) }
}
...
pub fn u8x4_to_u32(val: [u8; 4]) -> u32 {
    unsafe { std::mem::transmute::<[u8; 4], u32>(val) }
}
...
pub fn reduction_and(values: &[bool], inv: bool) -> bool {
    values
        .iter()
        .fold(true, |acc: bool, value: &bool| (inv ^ value) & acc)
}

pub fn reduction_or(values: &[bool], inv: bool) -> bool {
    values
        .iter()
        .fold(false, |acc: bool, value: &bool| (inv ^ value) | acc)
}
