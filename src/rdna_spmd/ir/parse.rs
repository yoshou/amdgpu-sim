...
impl<'a> Tokens<'a> {
        ...
        for (index, &b) in bytes.iter().enumerate() {
            ...
            if b.is_ascii_whitespace() || punct {
                if let Some(s) = start.take() {
                    items.push(&text[s..index]);
                }
                if punct {
                    items.push(&text[index..index + 1]);
                }
    ...
    fn next(&mut self) -> Result<&'a str> {
        let item = self
            .peek()
            .ok_or_else(|| format!("line {}: unexpected end", self.line))?;
        self.at += 1;
        Ok(item)
    }
    ...
    fn done(&self) -> Result<()> {
        if self.at == self.items.len() {
            ...
            Err(format!("line {}: trailing tokens", self.line))
        }
    }
    fn value(&mut self) -> Result<ValueId> {
        ...
        token
            ...
            .map(ValueId)
            .ok_or_else(|| format!("line {}: expected value, found {token:?}", self.line))
    }
    fn block(&mut self) -> Result<BlockId> {
        ...
        token
            ...
            .and_then(|n| n.parse().ok())
            .map(BlockId)
            .ok_or_else(|| format!("line {}: expected block, found {token:?}", self.line))
    }
    ...
    fn word<T: Copy>(&mut self, table: &[(&str, T)]) -> Result<T> {
        ...
        table
            ...
            .map(|x| x.1)
            .ok_or_else(|| format!("line {}: unknown keyword {token:?}", self.line))
    }
...
const FLOAT_OPS: &[(&str, FloatOp)] = &[
    ("add", FloatOp::Add),
    ("sub", FloatOp::Sub),
    ("mul", FloatOp::Mul),
    ("div", FloatOp::Div),
    ("minnum", FloatOp::MinNum),
    ("maxnum", FloatOp::MaxNum),
];
const FLOAT_UNARY: &[(&str, FloatUnary)] = &[("neg", FloatUnary::Neg), ("abs", FloatUnary::Abs)];
const FLOAT_PREDS: &[(&str, FloatPred)] = &[
    ("oeq", FloatPred::Oeq),
    ("ogt", FloatPred::Ogt),
    ("oge", FloatPred::Oge),
    ("olt", FloatPred::Olt),
...
const CVTS: &[(&str, Cvt)] = &[
    ("sitofp.rte", Cvt::SignedToFloatRte),
    ("uitofp.rte", Cvt::UnsignedToFloatRte),
    ("fptosi.sat.rtz", Cvt::FloatToSignedSatRtz),
    ("fptoui.sat.rtz", Cvt::FloatToUnsignedSatRtz),
    ("fpresize.rte", Cvt::FloatResizeRte),
    ("zext", Cvt::ZExt),
    ("sext", Cvt::SExt),
    ("trunc", Cvt::Trunc),
    ("bitcast", Cvt::Bitcast),
...
const SPACES: &[(&str, Space)] = &[
    ("global", Space::Global),
    ("scratch", Space::Scratch),
    ("lds", Space::Lds),
];
const SIZES: &[(&str, MemSize)] = &[
    ("u8", MemSize::U8),
    ("i8", MemSize::I8),
    ("u16", MemSize::U16),
    ("i16", MemSize::I16),
...
const SCOPES: &[(&str, Scope)] = &[
    ...
    ("cu", Scope::ComputeUnit),
    ("se", Scope::ShaderEngine),
    ("device", Scope::Device),
    ("system", Scope::System),
    ("workgroup", Scope::Workgroup),
];
const ORDERINGS: &[(&str, Ordering)] = &[
    ("relaxed", Ordering::Relaxed),
    ("acquire", Ordering::Acquire),
    ("release", Ordering::Release),
    ("seqcst", Ordering::Sequential),
];
const CACHE: &[(&str, CachePolicy)] = &[
    ("temporal", CachePolicy::Temporal),
    ("nontemporal", CachePolicy::NonTemporal),
    ("high", CachePolicy::HighPriority),
    ("lastuse", CachePolicy::LastUse),
    ("writeback", CachePolicy::WriteBack),
    ("near_nt", CachePolicy::NearNonTemporal),
    ("far_nt", CachePolicy::FarNonTemporal),
    ("near_nt_far_high", CachePolicy::NearNonTemporalFarHigh),
    ("near_nt_far_wb", CachePolicy::NearNonTemporalFarWriteBack),
];
const WAVES: &[(&str, WaveOp)] = &[
    ("any", WaveOp::Any),
    ("ballot", WaveOp::Ballot),
    ("readfirstlane", WaveOp::ReadFirstLane),
    ("readlane", WaveOp::ReadLane),
    ("writelane", WaveOp::WriteLane),
    ("bpermute", WaveOp::Bpermute),
    ("bpermute_fi", WaveOp::BpermuteFi),
    ("wmma", WaveOp::Wmma),
];
...
fn flag(t: &mut Tokens, name: &str) -> Result<bool> {
    ...
    match t.next()? {
        ...
        "1" => Ok(true),
        other => Err(format!(
            "line {}: expected {name}=0|1, found {other:?}",
            t.line
        )),
    }
}
    ...
    Ok(match kind {
        ...
        "unpack" => {
            ...
            match half {
                "lo" => Op::UnpackLo(a),
                "hi" => Op::UnpackHi(a),
                _ => return Err(format!("line {}: unknown half {half:?}", t.line)),
            }
        }
        "cmp" => {
            let p = t.word(INT_PREDS)?;
            let a = t.value()?;
            t.expect(",")?;
            Op::Cmp(p, a, t.value()?)
        }
        "float" => {
            let k = t.word(FLOAT_OPS)?;
            let a = t.value()?;
            t.expect(",")?;
            Op::Float(k, a, t.value()?)
        }
        "unary" => {
            let k = t.word(FLOAT_UNARY)?;
            Op::Unary(k, t.value()?)
        }
        "fcmp" => {
            let p = t.word(FLOAT_PREDS)?;
            let a = t.value()?;
            t.expect(",")?;
            Op::FCmp(p, a, t.value()?)
        }
        ...
        "convert" => {
            let k = t.word(CVTS)?;
            let to = t.ty()?;
            Op::Convert(k, to, t.value()?)
        }
        "const" => {
            let to = t.ty()?;
            Op::Const(to, t.number()?)
        }
        "select" => {
            let c = t.value()?;
            t.expect(",")?;
    ...
    Ok(match t.next()? {
        "memory" => {
            ...
            let op = match opcode {
                ...
                _ => {
                    let (kind, size) = opcode
                        .split_once('.')
                        .ok_or_else(|| format!("line {}: bad memory op {opcode:?}", t.line))?;
                    let size = SIZES
                        .iter()
                        .find(|(n, _)| *n == size)
                        .map(|x| x.1)
                        .ok_or_else(|| format!("line {}: bad size {size:?}", t.line))?;
                    match kind {
                        "load" => MemoryOp::Load(size),
                        "store" => MemoryOp::Store(size),
                        _ => return Err(format!("line {}: bad memory op {opcode:?}", t.line)),
                    }
                }
            };
        ...
        "barrier" => match t.next()? {
            "signal" => EffectOp::BarrierSignal {
                is_first: flag(t, "first")?,
            },
...
fn provenance(t: &mut Tokens) -> Result<u64> {
    ...
    token
        .strip_prefix("!p")
        .and_then(|n| n.parse().ok())
        .ok_or_else(|| format!("line {}: expected provenance, found {token:?}", t.line))
}
...
fn inst(registry: &DialectRegistry, t: &mut Tokens) -> Result<Inst> {
    ...
    let inst = match kind {
        "packet" => {
            let op = match t.next()? {
                "any" => PacketOp::Any,
                "ballot" => PacketOp::Ballot,
                other => return Err(format!("line {}: unknown packet op {other:?}", t.line)),
            };
            if lhs.len() != 1 || lhs[0].1 != op.result_type() {
                return Err(format!("line {}: packet result type", t.line));
            }
            Inst::Packet {
                op,
                input: t.value()?,
        ...
        "target" => {
            let name = t.next()?;
            let (dialect, operation) = name
                .split_once('.')
                .ok_or_else(|| format!("line {}: bad target name {name:?}", t.line))?;
            let dialect = registry
                .dialect_id(dialect)
                .ok_or_else(|| format!("line {}: unknown dialect {dialect:?}", t.line))?;
            let op = registry
                .lookup(dialect, operation)
                .map_err(|e| format!("line {}: {e}", t.line))?;
            let args = arguments(t.line, t.values_in_parens()?)?;
            let provenance = if t.peek().is_some() {
                Some(provenance(t)?)
            } else {
                None
            };
        ...
        _ => {
            ...
            if lhs.len() != 1 {
                return Err(format!(
                    "line {}: core instruction requires one result",
                    t.line
                ));
            }
            Inst::Core {
...
pub(crate) fn func(registry: &DialectRegistry, text: &str) -> Result<Func> {
    ...
    let mut define = |id: ValueId, ty: Ty, line: usize| -> Result<()> {
        if types.insert(id, ty).is_some() {
            return Err(format!("line {line}: duplicate definition of v{}", id.0));
        }
        Ok(())
    };
    ...
    for (line, text) in lines {
        ...
        let (_, block) = current
            .as_mut()
            .ok_or_else(|| format!("line {line}: instruction outside a block"))?;
        match t.peek() {
            Some("ret") => {
                t.next()?;
                ...
                t.done()?;
                block.term = Term::Ret(args);
            }
            Some("br") => {
                t.next()?;
                block.term = Term::Br(t.edge()?);
                t.done()?;
            }
            Some("condbr") => {
                t.next()?;
...
mod tests {
    ...
    fn every_operation_form_prints_and_parses_back_to_the_same_function() {
        ...
        let rcp = registry
            .lookup(crate::rdna_spmd::targets::rdna4::dialect::ID, "rcp.f32")
            .unwrap();
        let semantics = MemorySemantics {
            scope: Scope::Device,
            ordering: Ordering::Relaxed,
            cache_policy: CachePolicy::NearNonTemporalFarWriteBack,
            volatile: true,
        ...
        let d = core(
            &mut f,
            Ty::F64,
            Op::Convert(Cvt::FloatResizeRte, Ty::F64, fma),
        );
        ...
        insts.push(Inst::Packet {
            op: PacketOp::Any,
            input: cmp,
            output: any,
        });
        let ballot = f.value(Ty::I32);
        insts.push(Inst::Packet {
            op: PacketOp::Ballot,
            input: p2,
            output: ballot,
        });
        let t = f.value(Ty::F32);
        insts.push(Inst::Target {
            provenance: None,
            op: rcp,
            args: Arguments::Unary(p3),
            outputs: vec![(t, Ty::F32)],
        });
        ...
        insts.push(Inst::Effect {
            ...
            op: EffectOp::Memory {
                space: Space::Global,
                op: MemoryOp::Load(MemSize::I16),
                semantics,
            },
            inputs: vec![p1, p2],
            outputs: vec![(loaded, Ty::I32)],
        });
        insts.push(Inst::Effect {
            provenance: 8,
            op: EffectOp::Memory {
            ...
            inputs: vec![p0, loaded, p2],
            outputs: vec![],
        });
        let w = f.value(Ty::I32);
        insts.push(Inst::Effect {
            provenance: 9,
            op: EffectOp::Wave(WaveOp::ReadLane),
            inputs: vec![p0, loaded, k],
            outputs: vec![(w, Ty::I32)],
        });
        let first = f.value(Ty::I1);
        insts.push(Inst::Effect {
            provenance: 10,
            op: EffectOp::BarrierSignal { is_first: true },
            inputs: vec![p0],
            outputs: vec![(first, Ty::I1)],
        });
        insts.push(Inst::Effect {
            provenance: 11,
            op: EffectOp::BarrierWait,
            inputs: vec![w],
            outputs: vec![],
        });
        f.blocks.insert(
            BlockId(4),
            Block {
                params: vec![(p0, Ty::I32), (p1, Ty::I64), (p2, Ty::I1), (p3, Ty::F32)],
                insts,
                term: Term::CondBr {
                    cond: first,
                    yes: Edge {
    ...
    fn lifted_kernel_objects_round_trip_through_the_printer() {
        ...
        for &(path, symbol) in crate::rdna_spmd::targets::rdna4::decode::OBJECTS {
            let (entry, memory) =
                crate::rdna_spmd::targets::rdna4::decode::load_object(path, symbol);
