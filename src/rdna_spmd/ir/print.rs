...
pub(crate) fn cvt(op: Cvt) -> &'static str {
    match op {
        ...
        Cvt::FloatToSignedSatRtz => "fptosi.sat.rtz",
        Cvt::FloatToUnsignedSatRtz => "fptoui.sat.rtz",
        Cvt::FloatResizeRte => "fpresize.rte",
        Cvt::ZExt => "zext",
        Cvt::SExt => "sext",
        Cvt::Trunc => "trunc",
        Cvt::Bitcast => "bitcast",
...
pub(crate) fn cache_policy(c: CachePolicy) -> &'static str {
    match c {
        ...
        CachePolicy::HighPriority => "high",
        CachePolicy::LastUse => "lastuse",
        CachePolicy::WriteBack => "writeback",
        CachePolicy::NearNonTemporal => "near_nt",
        CachePolicy::FarNonTemporal => "far_nt",
        CachePolicy::NearNonTemporalFarHigh => "near_nt_far_high",
...
pub(crate) fn effect_op(op: EffectOp) -> String {
    match op {
        EffectOp::Memory {
            ...
            format!(
                "memory {} {} {} {} {} volatile={} deferred={}",
                op,
                space(s),
                scope(semantics.scope),
                ordering(semantics.ordering),
                cache_policy(semantics.cache_policy),
                semantics.volatile as u8,
                semantics.deferred_scope as u8
            )
        }
...
pub(crate) fn inst(registry: &DialectRegistry, types: &[Ty], i: &Inst) -> String {
    match i {
        Inst::Core {
            ...
            ty: t,
            op: o,
        } => format!("{}: {} = {}", v(*value), ty(*t), op(*o)),
        Inst::Packet {
            op: PacketOp::Any,
            input,
            output,
        } => format!("{}: i1 = packet any {}", v(*output), v(*input)),
        Inst::Packet {
            op: PacketOp::Ballot,
            input,
            output,
        } => format!("{}: i32 = packet ballot {}", v(*output), v(*input)),
        Inst::Target {
            provenance,
            op,
            args,
            ...
            let lhs = outputs
                .iter()
                .map(|&(id, t)| format!("{}: {}", v(id), ty(t)))
                .collect::<Vec<_>>()
                .join(", ");
            let provenance = provenance.map_or(String::new(), |p| format!(" !p{p}"));
            format!(
                "{lhs} = target {}({}){provenance}",
                target_name(registry, *op),
                values(args.values())
            )
        }
        Inst::Effect {
...
pub(crate) fn term(t: &Term) -> String {
    match t {
        ...
        Term::CondBr { cond, yes, no } => {
            format!("condbr {}, {}, {}", v(*cond), edge(yes), edge(no))
        }
    }
}
...
pub(crate) fn func(registry: &DialectRegistry, f: &Func) -> String {
    ...
    for (id, block) in &f.blocks {
        ...
        for i in &block.insts {
            writeln!(out, "  {}", inst(registry, &f.types, i)).unwrap();
        }
