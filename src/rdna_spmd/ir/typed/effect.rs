    ...
    pub fn verify(
        ...
        } = self
        {
            ...
            if matches!(op, MemoryOp::Load(_))
                && matches!(semantics.ordering, Ordering::Release | Ordering::AcqRel)
            {
                return Err("release load");
            }
            if matches!(op, MemoryOp::Store(_))
                && matches!(semantics.ordering, Ordering::Acquire | Ordering::AcqRel)
            {
...
#[cfg(test)]
mod tests {
    use super::super::cfg::{Block, BlockId, Func, Inst, Term};
    use super::*;
