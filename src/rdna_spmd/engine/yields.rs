...
mod tests {
    ...
    fn reduced_queries_match_wave_semantics_for_all_layouts_and_valid_lanes() {
        use crate::rdna_spmd::engine::fiber::{amdgpu_sim_fiber_yield_values, FiberCtx};
        ...
        let predicates: Vec<u32> = [0, 0x5555_5555, u32::MAX]
            .iter()
            .copied()
            .chain((0..32).map(|lane| 1 << lane))
            .collect();
                    ...
                    for source_kind in [
                        ...
                        for mask_kind in [
                            ...
                            for op in [WaveOp::Any, WaveOp::Ballot, WaveOp::ReadFirstLane] {
                                ...
                                layout.arguments = if op == WaveOp::ReadFirstLane {
                                    vec![source_kind, mask_kind]
                                } else {
                                    vec![mask_kind]
                                };
                                ...
                                for (index, &kind) in layout.arguments.iter().enumerate() {
                                    ...
                                    for lane in 0..32 {
                                        ...
                                        let representative = if matches!(kind, Argument::Uniform) {
                                            packet * width
                                        } else {
                                            lane
                                        };
                                        let value = match kind {
                                            ...
                                            _ if is_mask => {
                                                (predicate >> representative & 1) * 0x80000000
                                            }
                                        ...
                                        match kind {
                                            Argument::Constant(_) => {}
                                            Argument::Uniform => {
                                                frames[packet][(layout.base + index) * width] =
                                                    value
                                            }
                                            Argument::Lane if valid >> lane & 1 != 0 => {
                                                frames[packet]
                                                    [(layout.base + index) * width + lane % width] =
                                                    value
                                            }
                                ...
                                for (packet, fiber) in fibers.iter_mut().enumerate() {
                                    ...
                                    fiber.start(KernelArgs {
                                        entry: yield_frame as *const () as u64,
                                        sgprs: frames[packet].as_mut_ptr(),
                                        vgprs: std::ptr::null_mut(),
                                        spill: std::ptr::null_mut(),
                                        scratch_base: 0,
                                        scratch_stride: 0,
                                        lane_base: (packet * width) as u64,
                                        lds_base: 0,
                                        valid_mask: valid,
                                    });
                                ...
                                let expected =
                                    evaluate(op, valid, |index, lane| logical[index][lane])[0];
                                ...
                                for (packet, fiber) in fibers.iter_mut().enumerate() {
                                    ...
                                    for (cell, &value) in frames[packet].iter().enumerate() {
                                        let expected = if active && cell == layout.base * width {
                                            expected
                                        } else {
                                            before[packet][cell]
                                        };
    ...
    fn run_private(
        ...
        let kernel = if width == 0 {
            Compiler::default().compile_cooperative(program, 24)
        } else {
            Compiler::default().compile_cooperative_vec(program, 24, width as u32)
        };
        ...
        for packet in 0..count.div_ceil(width) {
            fibers[packet].start(KernelArgs {
                entry: kernel.addr(),
                sgprs: sgprs[packet].as_mut_ptr(),
                vgprs: vgprs[packet].as_mut_ptr(),
                spill: spill[packet].as_mut_ptr(),
                scratch_base,
                scratch_stride,
                lane_base: (packet * width) as u64,
                lds_base: 0,
        ...
        loop {
            let stops: Vec<_> = fibers[..count.div_ceil(width)]
                .iter_mut()
                .map(Fiber::resume)
                .collect();
            assert!(stops.iter().all(|pc| *pc == stops[0]));
            if stops[0] == FIBER_DONE {
                break;
            }
            for action in &kernel.yields[stops[0] as usize] {
                action.apply_wave(width, valid, &fibers);
            }
        }
    }
    ...
    fn private_pointer_broadcast_keeps_storage_private_to_each_lane() {
        ...
        use crate::rdna_instructions::{InstFormat, SOP1, SOP2, VFLAT, VOP1, VOP2};
        let read = |source, destination| YieldAction {
            op: EffectOp::Wave(WaveOp::ReadFirstLane),
            inputs: vec![
                Operand::Source(SourceOperand::VectorRegister(source)),
                Operand::Exec,
            ],
        ...
        let vcopy = |vdst, src| {
            InstFormat::VOP1(VOP1 {
                op: I::V_MOV_B32,
                vdst,
                src0: SourceOperand::ScalarRegister(src),
            })
        };
        let p = ScalarProgram {
            entry_pc: 0,
                    ...
                    ScalarBlock {
                        ...
                        body: vec![
                            ...
                            InstFormat::SOP1(SOP1 {
                                op: I::S_MOV_B64,
                                sdst: 4,
                                ssrc0: SourceOperand::PrivateBase,
                            }),
                            InstFormat::SOP2(SOP2 {
                                op: I::S_LSHR_B64,
                                sdst: 4,
                                ssrc0: SourceOperand::ScalarRegister(4),
                                ssrc1: SourceOperand::IntegerConstant(32),
                            }),
                            InstFormat::SOP2(SOP2 {
                                op: I::S_LSHL_B64,
                                sdst: 4,
                            ...
                            InstFormat::SOP1(SOP1 {
                                op: I::S_MOV_B32,
                                sdst: 126,
                                ssrc0: SourceOperand::LiteralConstant(1 << 21),
                            }),
                        ],
                        term: Terminator::Yield {
                            resume: 1,
                            action: Box::new(read(0, 10)),
                        },
                    },
                ),
                (
        ...
        for code_width in [0usize, 1, 2, 4, 8, 16] {
            ...
            for (count, base) in [(23usize, 0usize), (32, 32 * 16)] {
                ...
                for lane in 0..count {
                    s[lane / width][126] |= 1 << (lane % width);
                    v[lane / width][lane % width] = lane as u32;
                }
                run_private(
                    &p,
                    code_width,
                    ...
                    &mut s,
                    &mut v,
                    unsafe { scratch.as_mut_ptr().add(base) } as u64,
                    64,
                );
                for lane in 0usize..64 {
                    for word in 0..16 {
                        let local = lane.wrapping_sub(base / 16);
                        let expected = if local < count && word == 1 {
                            100 + local as u32
                        } else {
                            0xdeadbeef
                        };
                        assert_eq!(
                            scratch[lane * 16 + word],
                            expected,
                            "width={code_width} lane={lane} word={word}"
                        );
                    }
                }
            }
    ...
    fn memory_and_wave_results_use_wave_mask_words_before_saveexec() {
            ...
            blocks: BTreeMap::from([
                (
                    ...
                    ScalarBlock {
                        ...
                        term: Terminator::Yield {
                            resume: 1,
                            action: Box::new(YieldAction {
                                op: EffectOp::Wave(WaveOp::ReadLane),
                                inputs: vec![
                                    Operand::Source(SourceOperand::VectorRegister(2)),
                                    Operand::Source(SourceOperand::IntegerConstant(21)),
                                    Operand::Source(SourceOperand::IntegerConstant(2)),
                                ],
        ...
        for code_width in [0usize, 1, 2, 4, 8, 16] {
            ...
            for count in [1usize, 23, 32] {
                ...
                let mut v = vec![vec![0; 24 * width]; 32 / width];
                for packet in 0..count.div_ceil(width) {
                    s[packet][4] = ptr as u32;
                    s[packet][5] = (ptr >> 32) as u32;
                    s[packet][126] = (valid >> (packet * width)) & ((1 << width) - 1);
                    for lane in 0..width {
                        v[packet][2 * width + lane] = chosen;
                    }
                }
                ...
                for packet in 0..count.div_ceil(width) {
                    assert_eq!(
                        s[packet][10],
                        word & valid,
                        "width={code_width} count={count} packet={packet}"
                    );
                    assert_eq!(
                        s[packet][11], ballot,
                        "width={code_width} count={count} packet={packet}"
        ...
        let p = ScalarProgram {
            ...
            blocks: BTreeMap::from([
                ...
                (
                    ...
                    ScalarBlock {
                        pc: 1,
                        body: vec![mov(12, SourceOperand::IntegerConstant(1))],
                ...
                (
                    ...
                    ScalarBlock {
                        pc: 2,
                        body: vec![mov(12, SourceOperand::IntegerConstant(0))],
                        term: Terminator::Jump(3),
                    },
                ),
                (
                ...
                for mask in [0, 1 << 21, 0xaaaaaaaa, u32::MAX] {
                    ...
                    for (packet, regs) in s[..count.div_ceil(width)].iter().enumerate() {
                        ...
                        assert_eq!(
                            regs[10],
                            mask & valid,
                            "saved wave mask after entry validity clipping"
                        );
                        assert_eq!(regs[11], 0x80000001 & valid, "wave lane projection");
                        assert_eq!(
                            regs[12],
                            (mask & valid != 0) as u32,
                            "wave branch width={code_width} packet={packet}"
                        );
            ...
            for count in [23, 32] {
                for selector in [0u32, 17, 31, 63] {
                    ...
                    for packet in 0..32 / width {
                        s[packet][10] = 0x12345678;
                        s[packet][11] = selector;
                        // EXEC is empty throughout: WriteLane still applies.
                        for lane in 0..width {
                            v[packet][lane] = 100 + (packet * width + lane) as u32;
                        }
                    }
                    run(&p, code_width, count, &mut s, &mut v);
                    ...
                    for lane in 0..count {
                        ...
                        assert_eq!(
                            s[lane / width][12],
                            if selected < count { 0x12345678 } else { 0 }
                        );
                    }
                }
            }
    ...
    fn native_any_and_ballot_collect_full_wave_with_empty_exec_and_padding() {
        let p = program(vec![
            YieldAction::new(
                ...
                vec![Operand::Exec],
                vec![Destination::Sgpr(10)],
            ),
            YieldAction::new(
                EffectOp::Wave(WaveOp::Ballot),
                vec![Operand::Exec],
                vec![Destination::Sgpr(11)],
            ),
        ]);
        for code_width in [0, 1, 2, 4, 8, 16] {
            let width = code_width.max(1);
            for count in [1usize, 23, 32] {
                ...
                for mask in [0, 1, 1 << 21, 1 << 31, 0xaaaaaaaa, u32::MAX] {
                    ...
                    for regs in &s[..count.div_ceil(width)] {
                        assert_eq!(
                            (regs[10], regs[11]),
                            ((mask & valid != 0) as u32, mask & valid),
                            "width={}, count={}, mask={:#x}",
                            width,
                            count,
                            mask
    ...
    fn native_wmma_frames_preserve_float_accumulators_and_source_overlap() {
        ...
        let p = program(vec![
            YieldAction::new(
                ...
                args(8),
                (0..8).map(Destination::Vgpr).collect(),
            ),
            YieldAction::new(
                EffectOp::Wave(WaveOp::Wmma),
                (16..24).map(v).chain((0..8).map(v)).collect(),
                (8..16).map(Destination::Vgpr).collect(),
            ),
        ]);
        for code_width in [0, 1, 2, 4, 8, 16] {
            let width = code_width.max(1);
            for count in [23usize, 32] {
                ...
                for lane in 0..count {
                    for m in 0..8 {
                        ...
                        assert_eq!(
                            regs[lane / width][m * width + lane % width],
                            (c + 2.0 * terms).to_bits()
                        );
                        assert_eq!(
                            regs[lane / width][(8 + m) * width + lane % width],
                            (c + 4.0 * terms).to_bits()
                        );
                    }
                }
