...
impl Operation {
    ...
    pub fn emit(&self, emitter: &super::codegen::ops::Emitter, args: &[Value]) -> Vec<Value> {
        ...
        assert_eq!(
            values.len(),
            self.outputs.len(),
            "provider result count mismatch"
        );
        values
    }
}
...
impl DialectRegistry {
    ...
    pub fn add_idiom(&mut self, idiom: Box<dyn super::pass::idioms::Idiom>) {
        self.idioms.push(idiom);
    }
    pub fn registers(&self) -> Registers {
        self.registers
    }
    pub fn idioms(&self) -> &[Box<dyn super::pass::idioms::Idiom>] {
        &self.idioms
    }
    pub fn lowering_state(
        &self,
        emitter: &super::codegen::ops::Emitter,
        sink: Value,
    ...
    #[cfg(test)]
    pub fn dialect_id(&self, name: &str) -> Option<u32> {
        self.dialects
            .iter()
    ...
    pub fn result_type(
        ...
        match self.result_types(op, args, types)? {
            [ty] => Ok(*ty),
            _ => Err("operation has multiple results"),
        }
    }
}
...
mod tests {
    ...
    fn registry_rejects_collisions_missing_providers_and_bad_types() {
        ...
        assert_eq!(
            registry.result_type(id, Arguments::Unary(ValueId(0)), &[Ty::F32]),
            Ok(Ty::F32)
        );
        assert!(registry
            .result_type(id, Arguments::Unary(ValueId(0)), &[Ty::I32])
            .is_err());
        assert!(registry
            .result_type(id, Arguments::Unary(ValueId(1)), &[Ty::F32])
            .is_err());
    }
    ...
    fn target_provider_matches_reference_at_all_widths() {
        ...
        use crate::rdna_spmd::{codegen::ops::Emitter, native::jit};
        use std::sync::Arc;
        ...
        for opcode in [
            ...
            let target = rdna4::unary(&registry, opcode)
                .or_else(|| rdna4::binary(&registry, opcode))
                .unwrap();
            let spec = registry.operation(target).unwrap();
            let input_ty = spec.inputs[0];
            let output_ty = spec.outputs[0];
            let inputs: Vec<u64> = if input_ty == Ty::I32 {
                vec![
                    0, 0x8000, 1, 0x83ff, 0x0400, 0x3c00, 0x3e00, 0xbc00, 0x7bff, 0x7c00, 0xfc00,
                    0x7d23, 0xfe45, 0xabcd3c00,
                ]
            } else if input_ty == Ty::F32 {
                vec![
                    0, 0x80000000, 1, 0x807fffff, 0x800000, 0x3f800000, 0x3fc00000, 0x40200000,
                    0xbf000000, 0xc0200000, 0x7f7fffff, 0x7f800000, 0xff800000, 0x7fa12345,
                    0xffc12345, 0x3e800000, 0xbe800000, 0x3f000000, 0x40490fdb,
                ]
            } else {
                vec![
                    0,
                    ...
                    0xfff8234512341234,
                    0x3fd000000000000b,
                ] // Witness against successive subnormal rounding.
            };
            let binary = spec.inputs.len() == 2;
            let exponents = if binary {
                vec![
                    i32::MIN,
                    -2200,
                    -1075,
            ...
            let cases: Vec<_> = inputs
                .iter()
                .flat_map(|&bits| exponents.iter().map(move |&exp| (bits, exp)))
                .collect();
            for width in [0, 1, 2, 4, 8, 16] {
                unsafe {
                    ...
                    let pointer = ir.ptr();
                    let f =
                        ir.add_function("kernel", ir.void().function(&[pointer, pointer, pointer]));
                    ...
                    let run: unsafe extern "C" fn(*const u32, *const i32, *mut u32) =
                        std::mem::transmute(code.address() as usize);
                    let w = width.max(1) as usize;
                    let input_words = input_ty.bits() as usize / 32;
                    let output_words = output_ty.bits() as usize / 32;
                    let mut src = vec![0u32; w * input_words];
                    let mut dst = vec![0u32; w * output_words];
                    ...
                    for start in (0..cases.len()).step_by(w) {
                        for lane in 0..w {
                            ...
                            if input_words == 2 {
                                src[lane * input_words + 1] = (bits >> 32) as u32;
                            }
                        }
                        run(src.as_ptr(), exp.as_ptr(), dst.as_mut_ptr());
                        for lane in 0..w {
                            let got = dst[lane * output_words] as u64
                                | if output_words == 2 {
                                    (dst[lane * output_words + 1] as u64) << 32
                                } else {
                                    0
                                };
                            let (bits, exponent) = cases[(start + lane) % cases.len()];
                            ...
                            let expected = if matches!(opcode, I::V_TRIG_PREOP_F64) {
                                ...
                                if input_ty == Ty::F32 {
                                    libm::scalbnf(f32::from_bits(bits as u32), exponent).to_bits()
                                        as u64
                                } else {
                                    libm::scalbn(f64::from_bits(bits), exponent).to_bits()
                                }
                            } else {
                                rdna4::reference(opcode, bits)
                            };
                            let nan = |bits| match output_ty {
                                Ty::F32 => f32::from_bits(bits as u32).is_nan(),
                                Ty::F64 => f64::from_bits(bits).is_nan(),
                                _ => false,
                            };
                            assert!(
