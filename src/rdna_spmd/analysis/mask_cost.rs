...
fn loop_depths(f: &Func) -> BTreeMap<BlockId, u32> {
    ...
    for (&id, block) in &f.blocks {
        for e in block.term.edges() {
            preds.entry(e.dst).or_default().push(id);
        }
    }
...
pub(crate) fn costs(f: &Func, width: u32, host: &Vectors) -> (f64, f64) {
    ...
    let (narrow_pack, narrow_unpack, narrow_logic, narrow_regs) = if !vector {
        (0.0, 0.0, 1.0, 1.0)
    } else if host.native_masks {
        (0.0, 0.0, 1.0, 0.0)
    } else {
        (2.0 * ops(64), 2.0 * ops(64), ops(32), ops(32))
    };
    let (wide_widen, wide_pack, wide_logic, wide_regs) = if !vector {
        (0.0, 0.0, 1.0, 1.0)
    } else if host.native_masks {
        (ops(64), ops(64), ops(64), ops(64))
    } else {
        (ops(64), 2.0 * ops(64), ops(64), ops(64))
    };
    ...
    for block in f.blocks.values() {
        for inst in &block.insts {
            if let Inst::Core {
                value,
                op: Op::Cmp(_, a, _) | Op::FCmp(_, a, _),
                ..
            } = inst
            {
                produced[value.0] = Some(f.types[a.0].bits());
            }
        }
    }
    ...
    for (&id, block) in &f.blocks {
        ...
        for inst in &block.insts {
            match inst {
                ...
                Inst::Core {
                    ty: Ty::I1,
                    op: Op::Int(IntOp::And | IntOp::Or | IntOp::Xor, ..) | Op::Select(..),
                    ..
                } => logic += 1,
                Inst::Core {
                    ty: Ty::I1,
                    op: Op::Convert(Cvt::Trunc, ..),
                    ..
                } => {}
        ...
        for inst in &block.insts {
            super::super::pass::dce::operands(inst, |v| *uses.entry(v).or_default() += 1);
        }
        for e in block.term.edges() {
            for &v in &e.args {
        ...
        for (&v, &count) in &uses {
            ...
            let escapes = count > same
                || (bits == 64 && selects32.contains(&v))
                || (bits == 32 && selects64.contains(&v));
            if !escapes {
                continue;
            }
            if bits == 64 {
