...
impl<M> Uniformity<M> {
    ...
    pub fn pair(&self, lo: ValueId, hi: ValueId) -> Option<Fact> {
...
mod tests {
    ...
    use crate::rdna_spmd::{
        targets::rdna4::decode::{ScalarBlock, ScalarProgram, Terminator},
        targets::rdna4::lift::wave::{Destination, Operand, YieldAction},
        CompilationInput,
    };
    ...
    fn parameter(
        f: &crate::rdna_spmd::program::LiftedFunction,
        pc: usize,
        register: u32,
    ) -> ValueId {
    ...
    fn returned(
        ...
        let Term::Ret(args) = &f.ir.blocks[&BlockId(pc)].term else {
            panic!("block does not return")
        };
    ...
    fn scratch_loads_are_varying_and_uniform_global_loads_are_uniform() {
        let scratch = InstFormat::VSCRATCH(VSCRATCH {
        ...
        let global = InstFormat::VGLOBAL(VGLOBAL {
    ...
    fn cross_lane_writes_on_resume_are_varying_and_break_frames() {
        let write = |reg| {
            Box::new(YieldAction::new(
                crate::rdna_spmd::ir::EffectOp::Wave(crate::rdna_spmd::ir::WaveOp::WriteLane),
                vec![
                    Operand::Source(SourceOperand::IntegerConstant(0)),
                    Operand::Source(SourceOperand::IntegerConstant(0)),
                    Operand::Source(SourceOperand::VectorRegister(reg)),
                    Operand::Source(SourceOperand::IntegerConstant(reg as u64)),
                ],
        ...
        assert_eq!(
            u.pair(parameter(&f, 2, 10), parameter(&f, 2, 11)),
            Some(Fact::Affine {
                stride: 16,
                span: None
            })
    ...
    fn affine_lane_indices_scale_by_constants_and_survive_uniform_offsets() {
        ...
        f.blocks.insert(
            BlockId(0),
            Block {
                params: vec![(lane, Ty::I32), (base, Ty::I32), (flag, Ty::I1)],
                insts,
                term: Term::Ret(vec![]),
            },
        );
    ...
    fn packed_work_item_ids_split_into_an_affine_x_and_uniform_y() {
        ...
        f.blocks.insert(
            BlockId(0),
            Block {
                params: vec![(ids, Ty::I32), (other, Ty::I32)],
                insts,
                term: Term::Ret(vec![]),
            },
        );
    ...
    fn block_arguments_meet_incoming_facts_including_split_pairs() {
        ...
        let p_lo = f.value(Ty::I32);
        let p_hi = f.value(Ty::I32);
        let p_base = f.value(Ty::I32);
        let p_mixed = f.value(Ty::I32);
        f.blocks.insert(
            BlockId(0),
            Block {
                params: vec![(lane, Ty::I32), (base, Ty::I32)],
                insts,
                term: Term::CondBr {
                    cond,
                    yes: Edge {
                        dst: BlockId(1),
                        args: vec![lo, hi, base, base],
                    },
                    no: Edge {
                        dst: BlockId(1),
                        args: vec![lo, hi, one, lane],
                    },
                },
            },
        );
        ...
        f.blocks.insert(
            ...
            Block {
                ...
                insts: vec![Inst::Core {
                    value: next,
                    ty: Ty::I32,
                    op: Op::Int(IntOp::Add, p_base, one),
                }],
                term: Term::CondBr {
                    cond,
                    yes: Edge {
                        dst: BlockId(1),
                        args: vec![p_lo, p_hi, next, p_mixed],
                    },
                    no: Edge {
                        dst: BlockId(2),
                        args: vec![],
