...
fn quotient_ir(
    target: crate::rdna_spmd::dialect::TargetOp,
    kind: u8,
    ...
    if kind == 7 {
        quotient = push(Ty::F64, Op::Convert(Cvt::Bitcast, Ty::F64, quotient));
    }
    let denominator = if matches!(kind, 6 | 8) {
        ...
        if kind == 6 {
            ...
            push(Ty::F64, Op::Select(predicate, den, num))
        }
    } else {
        den
    };
    ...
    insts.push(Inst::Target {
        provenance: None,
        op: target,
        args: Arguments::Ternary([quotient, denominator, num]),
        outputs: vec![(result, Ty::F64)],
    });
    f.blocks.insert(
        BlockId(0),
        Block {
            params: vec![(den, Ty::F64), (num, Ty::F64)],
            insts,
...
fn proven_quotient_matches_general_fixup_and_other_quotients_keep_isa_corrections() {
    ...
    for width in [0, 1, 2, 4, 8, 16, 32] {
        unsafe {
                ...
                for inst in &block.insts {
                    match inst {
                        ...
                        Inst::Target {
                            ...
                            for ((value, _), result) in outputs.iter().zip(results) {
                                values[value.0] = result;
                            }
                        }
                        _ => unreachable!(),
                    }
            ...
            b.ret_void();
            let code = module.finish(if width == 0 {
                jit::Mode::Scalar
            } else {
                jit::Mode::Packet
            });
            let run: unsafe extern "C" fn(*const u64, *const u64, *mut u64, *mut u64) =
                std::mem::transmute(code.address());
            let mut den = vec![0; lanes];
            let mut num = vec![0; lanes];
            let mut general = vec![0; lanes * programs.len()];
            let mut specialized = general.clone();
            for start in (0..cases.len()).step_by(lanes) {
                for lane in 0..lanes {
                    (den[lane], num[lane]) = cases[(start + lane) % cases.len()];
                }
                run(
                    den.as_ptr(),
                    num.as_ptr(),
    ...
    for width in [0, 1, 2, 4, 8, 16] {
        unsafe {
            ...
            for (index, &input_ty) in spec.inputs.iter().enumerate() {
                let storage_ty = if input_ty == Ty::I1 {
                    Ty::I32
                } else {
                    input_ty
                };
                let value = b
                    .load(e.ty(storage_ty), f.param(index as u32))
                    .set_alignment(4);
                values.push(if input_ty == Ty::I1 {
                    b.trunc(value, e.ty(Ty::I1))
                } else {
                    value
                });
            }
            let args = if values.len() == 4 {
                Arguments::Quaternary([ValueId(0), ValueId(1), ValueId(2), ValueId(3)])
            } else {
                Arguments::Ternary([ValueId(0), ValueId(1), ValueId(2)])
            };
            assert_eq!(
                registry.result_types(target, args, spec.inputs).unwrap(),
                spec.outputs
            );
            let results = e.target(target, args, &values);
            for (index, &value) in results.iter().enumerate() {
                let value = if index == 1 {
                    b.zext(value, e.ty(Ty::I32))
                } else {
                    value
                };
            ...
            let lanes = width.max(1) as usize;
            let words = ty.bits() as usize / 32;
            let mut inputs: [Vec<u32>; 4] = std::array::from_fn(|_| vec![0; lanes * words]);
            let mut result = vec![0u32; lanes * words];
            let mut flags = vec![0u32; lanes];
            for start in (0..cases.len()).step_by(lanes) {
                for lane in 0..lanes {
                    ...
                    for input in 0..4 {
                        ...
                        if stride == 2 {
                            inputs[input][lane * stride + 1] = (case.inputs[input] >> 32) as u32;
                        }
                    }
                }
                run(
                ...
                for lane in 0..lanes {
                    let case = &cases[(start + lane) % cases.len()];
                    let bits = result[lane * words] as u64
                        | if words == 2 {
                            (result[lane * words + 1] as u64) << 32
                        } else {
                            0
                        };
                    assert_eq!(
                        ...
                        case.result,
                        "{opcode:?} W{width} case {}",
                        (start + lane) % cases.len()
                    );
                    if results.len() == 2 {
                        assert_eq!(flags[lane], case.flag, "{opcode:?} W{width} flag");
                    }
                }
            }
        }
    ...
    for (op, one, rows) in [
        (
            ...
            vec![
                ...
                (0, 0xffc0_0000, 1),
                (0x8000_0000, 0xffc0_0000, 1),
                (0x7f80_0000, 0x7f80_0000, 1),
                (0xff80_0000, 0xff80_0000, 1),
                (0x7fc0_0000, 0x7fc0_0000, 1),
                (1, 0x1500_0000, 1),
                (0x0080_0000, 0x2080_0000, 1),
                (0x7f7f_ffff, 0x5f7f_ffff, 1),
                (0x3f00_0000, 0x3f00_0000, 0),
            ],
        ),
        (
            I::V_DIV_SCALE_F64,
            0x3ff0_0000_0000_0000,
            vec![
                (0x4000_0000_0000_0000, 0x4000_0000_0000_0000, 0),
                (0, 0xfff8_0000_0000_0000, 1),
                ...
                (1, 0x04d0_0000_0000_0000, 1),
                (0x0010_0000_0000_0000, 0x0810_0000_0000_0000, 1),
        ...
        for (den, result, flag) in rows {
            let ty = if matches!(op, I::V_DIV_SCALE_F32) {
                Ty::F32
            } else {
                Ty::F64
            };
            ...
            cases.push(Case {
                inputs: [den, den, one, 0],
                result,
                flag,
            });
            let result = if den == 0 || den == 0x8000_0000 || den == 0x8000_0000_0000_0000 {
                result
            } else {
                one
            };
            cases.push(Case {
                inputs: [one, den, one, 0],
                result,
                flag,
            });
        }
    ...
    for (op, ty, inputs) in [
        ...
        (
            ...
            vec![
                ...
                0x6fe0_0000_0000_0000,
                0x6ff0_0000_0000_0000,
                0x7fc0_0000_0000_0000,
...
fn fmas_is_fused_and_consumes_the_lane_flag() {
    for (op, ty, exponent) in [
        (I::V_DIV_FMAS_F32, Ty::F32, 32),
        (I::V_DIV_FMAS_F64, Ty::F64, 64),
    ] {
        let mut cases = Vec::new();
        for [a, b, c] in [
            [1.5f64, -2., 4.],
            ...
            for flag in [0, 1] {
                ...
                cases.push(Case {
                    inputs: [encode(a), encode(b), encode(c), flag],
                    result,
                    flag: 0,
                });
            }
