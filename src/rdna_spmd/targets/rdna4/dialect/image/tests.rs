...
fn reference(
    ...
    let one = if matches!(format, 1 | 2) {
        1f32.to_bits()
    } else {
        1
    };
    if selector < 2 {
        return if selector == 0 { 0 } else { one };
    }
    let index = |value: f32, extent: i32| {
        ...
        match mode {
            ...
            1 => {
                let x = n.rem_euclid(2 * extent);
                Some(if x < extent { x } else { 2 * extent - 1 - x })
            }
    ...
    let (Some(x), Some(y)) = (index(u, 5), index(v, 3)) else {
        return one;
    };
...
fn field(words: &mut [u32], bit: usize, count: usize, value: u32) {
    for k in 0..count {
        words[(bit + k) / 32] |= ((value >> k) & 1) << ((bit + k) % 32);
    }
}
...
fn image_ir_requires_constant_component_and_unique_effect_provenance() {
    ...
    for case in 0..4 {
        ...
        match case {
            0 => {
                block.insts.remove(0);
                block.params.push((ValueId(12), Ty::I32));
            }
            1 => {
                if let Inst::Core { op, .. } = &mut block.insts[0] {
                    *op = Op::Const(Ty::I32, 4);
                }
            }
            2 => {
                if let Inst::Target { provenance, .. } = &mut block.insts[1] {
                    *provenance = None;
                }
            }
            3 => {
                bad.types.push(Ty::I32);
                block.insts.push(Inst::Target {
                    provenance: Some(7),
                    op,
                    args,
                    outputs: vec![(ValueId(17), Ty::I32)],
                });
            }
            _ => unreachable!(),
        }
...
fn native_sampler_matches_reference_with_lane_distinct_descriptors_and_coordinates() {
    ...
    for y in 0..3 {
        for x in 0..5 {
            texels[y * 128 + x] = (128 + x * 13 + y * 11) as u8;
        }
    }
    ...
    let cases = [1, 2, 5, 6]
        .iter()
        .flat_map(|&format| {
            (0..8).flat_map(move |mode| {
                [false, true].iter().copied().flat_map(move |unrm| {
                    (0..coordinates.len()).map(move |j| (format, mode, unrm, j))
                })
            })
        })
        .collect::<Vec<_>>();
    for width in [0, 1, 2, 4, 8, 16] {
        for component in 0..4 {
            unsafe {
                ...
                let result = e.target(
                    target,
                    Arguments::Sixteen(std::array::from_fn(ValueId)),
                    &values,
                )[0];
                ...
                let run: unsafe extern "C" fn(*const u32, *mut u32) =
                    std::mem::transmute(code.address() as usize);
                let mut input = vec![0u32; 16 * w];
                ...
                for start in (0..cases.len()).step_by(w) {
                    ...
                    for lane in 0..w {
                        ...
                        desc[0] = (address >> 8) as u32;
                        desc[1] = (address >> 40) as u32;
                        field(&mut desc, 49, 8, format);
                        field(&mut desc, 62, 16, 4);
                        field(&mut desc, 78, 16, 2);
                        let selector = [0, 1, 4, 7][(j + component as usize) % 4];
                        field(&mut desc, 96 + component as usize * 3, 3, selector);
                        field(&mut desc[8..12], 0, 3, mode);
                        field(&mut desc[8..12], 3, 3, mode);
                        field(&mut desc[8..12], 126, 2, 2);
                        desc[13] = unrm as u32;
                        desc[14] = coordinates[j].to_bits();
                        desc[15] = coordinates[(j + 3) % coordinates.len()].to_bits();
                        ...
                        expected.push(reference(
                            ...
                            mode,
                            unrm,
                            f32::from_bits(desc[14]),
                            f32::from_bits(desc[15]),
                            selector,
                        ));
                    }
                    ...
                    assert_eq!(
                        output, expected,
                        "width={width} component={component} start={start}"
                    );
                }
            }
        }
