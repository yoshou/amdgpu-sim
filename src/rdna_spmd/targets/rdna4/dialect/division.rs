...
fn scale_operand(e: &Emitter, ty: Ty, a: &[Value]) -> Vec<Value> {
    ...
    let exponent = |value| {
        ...
        if word == Ty::I64 { ir.trunc(bits, e.ty(Ty::I32)) } else { bits }
    };
    ...
    let operands_tiny = ir.or(ir.icmp(IntPred::Eq, den, k(0)), ir.icmp(IntPred::Sle, num, k(tiny)));
    let zero = e.constant(ty, 0);
    let invalid = ir.or(ir.fcmp(FloatPred::Oeq, a[1], zero), ir.fcmp(FloatPred::Oeq, a[2], zero));
...
pub(super) fn scale_f32(e: &Emitter, a: &[Value]) -> Vec<Value> { scale_operand(e, Ty::F32, a) }
pub(super) fn scale_f64(e: &Emitter, a: &[Value]) -> Vec<Value> { scale_operand(e, Ty::F64, a) }
...
pub(super) fn fmas_f32(e: &Emitter, a: &[Value]) -> Value { fmas(e, Ty::F32, a) }
pub(super) fn fmas_f64(e: &Emitter, a: &[Value]) -> Value { fmas(e, Ty::F64, a) }

pub(super) fn fixup_f32(e: &Emitter, a: &[Value]) -> Value { fixup(e, Ty::F32, a) }
fn fixup(e: &Emitter, ty: Ty, a: &[Value]) -> Value {
    ...
    let bits = a[..3].iter().map(|&a| ir.bitcast(a, e.ty(word))).collect::<Vec<_>>();
    ...
    let tiny = ir.icmp(IntPred::Slt, delta, e.constant(Ty::I32, underflow as u32 as u64));
...
pub(super) fn fixup_f64(e: &Emitter, a: &[Value]) -> Value { fixup(e, Ty::F64, a) }
