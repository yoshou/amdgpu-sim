...
fn operands(inst: &InstFormat) -> Option<Operands> {
    let (op, mut src, mut dst, abs, neg, clamp, omod, mut src_high, mut dst_high, short) =
        match inst {
    ...
    let widening = match op {
        I::V_CVT_F32_F16 => true,
        I::V_CVT_F16_F32 => false,
        _ => return None,
    };
    if short {
        if widening {
            if let SourceOperand::VectorRegister(ref mut r) = src {
                src_high = *r & 128 != 0;
                *r &= 127;
            }
        } else {
...
pub(in crate::rdna_spmd) fn registers(inst: &InstFormat) -> Option<(Vec<u32>, u32)> {
    if let InstFormat::VOP3P(i) = inst {
        if matches!(i.op, I::V_FMA_MIXLO_F16 | I::V_FMA_MIXHI_F16) {
            let mut reads: Vec<_> = [i.src0, i.src1, i.src2]
                .iter()
                .filter_map(|s| {
                    if let SourceOperand::VectorRegister(r) = s {
                        Some(*r as u32)
                    } else {
                        None
                    }
    ...
    if let SourceOperand::VectorRegister(r) = o.src {
        reads.push(r as u32);
    }
    if !o.widening {
        reads.push(o.dst as u32);
    }
...
pub(super) fn instruction(inst: &InstFormat, registry: &DialectRegistry) -> Option<Lowering> {
    ...
    if widening {
        ...
        let mut b = Builder::new(
            ...
            vec![
                input(src, Ty::F32),
                input(SourceOperand::VectorRegister(dst), Ty::I32),
            ],
        );
        ...
        value = b.target_one(target, Arguments::Unary(value));
        if omod != 0 {
            let mask = b.k(Ty::I32, 0x7c00);
            let exponent = b.int(IntOp::And, value, mask);
