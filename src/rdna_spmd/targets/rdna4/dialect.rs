...
pub(in crate::rdna_spmd) fn reference_class(ty: Ty, bits: u64, selector: u32) -> bool {
    let (fraction_bits, exponent_bits, sign_bit) = match ty {
        Ty::F32 => (23, 8, 31),
        Ty::F64 => (52, 11, 63),
        _ => unreachable!(),
    };
...
pub(in crate::rdna_spmd) fn unary(registry: &DialectRegistry, op: I) -> Option<TargetOp> {
    let name = match op {
        ...
        I::V_EXP_F32 => "exp2.f32",
        I::V_LOG_F32 => "log2.f32",
        I::V_SIN_F32 => "sin.f32",
        I::V_COS_F32 => "cos.f32",
        _ => return None,
    };
...
pub(in crate::rdna_spmd) fn reference(op: I, bits: u64) -> u64 {
    ...
    match op {
        I::V_RCP_F32 | I::V_RCP_IFLAG_F32 | I::V_S_RCP_F32 => {
            flush(1.0 / flush(f32::from_bits(bits as u32))).to_bits() as u64
        }
        ...
        I::V_SIN_F32 | I::V_COS_F32 => {
            ...
            let result = if matches!(op, I::V_SIN_F32) {
                if x == 0.0 {
                    x
                } else if r == 0.0 || r.abs() == 0.5 {
                    0.0
                } else {
                    (r * std::f64::consts::TAU).sin() as f32
                }
            } else if r.abs() == 0.25 {
                0.0
            } else {
                (r * std::f64::consts::TAU).cos() as f32
            };
        ...
        | I::V_FREXP_EXP_I32_F64 => {
            ...
            let fraction = bits & ((1 << frac) - 1);
            let exp = ((bits & !sign) >> frac) as i32;
            if exp == bias * 2 + 1 {
                if mant {
                    bits | if fraction != 0 { 1 << (frac - 1) } else { 0 }
                } else {
                    0
                }
                ...
                if mant {
                    sign | (((bias - 1) as u64) << frac) | ((fraction << shift) & ((1 << frac) - 1))
                } else {
                    (2 - bias - shift as i32) as u32 as u64
                }
            } else if mant {
                sign | (((bias - 1) as u64) << frac) | fraction
            } else {
                (exp - bias + 1) as u32 as u64
            }
        }
        _ => unreachable!(),
    }
