...
impl<'a> Cg<'a> {
    ...
    pub(super) fn emit_local_wave(&mut self, op: EffectOp, inputs: &[ValueId], outputs: &[(ValueId, Ty)]) {
        let ir = self.ir;
        match op {
            EffectOp::Wave(WaveOp::Any) | EffectOp::Wave(WaveOp::Ballot) => {
                let bits = match self.p.width {
                    ...
                    None => { let v = self.scalar(inputs[0]); ir.zext(v, ir.i32()) }
                };
                let result = if op == EffectOp::Wave(WaveOp::Any) { ir.icmp(IntPred::Ne, bits, self.ci32(0)) } else { bits };
            ...
            EffectOp::Wave(WaveOp::ReadFirstLane) => {
                let result = if let Some(w) = self.p.width {
                    ...
                    let tz = ir.call_named("llvm.cttz.i32", ir.i32(), &[ir.i32(), ir.i1()], &[word, ir.ci1(false)]);
    ...
    pub(super) fn emit_yield(&mut self, members: &[(u64, Vec<ValueId>, Vec<(ValueId, Ty)>)]) {
        ...
        let pointer = |slot: usize| ir.gep(ir.i32(), frame, &[ir.ci32((slot as u64 * width) as u32)]);
        for (provenance, inputs, _) in members {
            ...
            for (i, (&id, &ty)) in inputs.iter().zip(&layout.inputs).enumerate() {
                ...
                let int_ty = if uniform || self.p.width.is_none() { ir.i32() } else { self.vi32() };
                let bits = match ty {
        ...
        for (provenance, _, outputs) in members {
            ...
            for (i, &(id, ty)) in outputs.iter().enumerate() {
                let int_ty = if uniform || self.p.width.is_none() { ir.i32() } else { self.vi32() };
                let bits = ir.load(int_ty, pointer(layout.base + layout.output_base + i)).set_alignment(4);
                let bool_ty = if uniform || self.p.width.is_none() { ir.i1() } else { self.vi1() };
                let f32_ty = if uniform || self.p.width.is_none() { ir.f32() } else { self.vec_ty(ir.f32()) };
                let result = match ty {
