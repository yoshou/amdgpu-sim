    ...
    for id in ids {
        ...
        for (index, inst) in insts.iter_mut().enumerate() {
            ...
            let mut substitute = |v: ValueId, f: &mut Func, fresh: &mut Vec<Inst>| {
                if let Some(&hit) = memo.get(&(v, current)) {
                    return hit.unwrap_or(v);
                }
            ...
            match inst {
                Inst::Core { value, op, .. } => {
                    if masks.exposed.get(value.0).copied().unwrap_or(0) == 0 {
                        ...
                        match *op {
                            Op::Select(c, new, old)
                                if masks.predicated.get(value.0).is_some_and(|p| p.is_some()) =>
                            {
                                *op = Op::Select(c, substitute(new, f, &mut fresh), old)
                            }
...
mod tests {
    use super::super::super::ir::{
        CachePolicy, EffectOp, Env, IntOp, IntPred, MemSize, MemoryOp, MemorySemantics, Ordering,
        Scope, Space, Ty,
    };
    use super::*;
    ...
    impl Builder {
        fn core(&mut self, ty: Ty, op: Op) -> ValueId {
            let v = self.f.value(ty);
            self.insts.push(Inst::Core { value: v, ty, op });
            v
        }
        fn ballot(&mut self, bit: ValueId) -> ValueId {
            let v = self.f.value(Ty::I32);
            self.insts.push(Inst::Packet {
                op: PacketOp::Ballot,
                input: bit,
                output: v,
            });
    ...
    fn masks(f: &Func) -> std::rc::Rc<Masks> {
        ...
        Analyses::new(super::super::super::analysis::Context::new(
            &registry,
            &[],
            0,
            16,
    ...
    fn unobserved_predicated_writes_lose_their_select_and_exposed_ones_keep_it() {
        ...
        let old = b.f.value(Ty::I32);
        let address = b.f.value(Ty::I32);
