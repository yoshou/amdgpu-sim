...
fn half(defs: &[Option<Op>], v: ValueId) -> Option<(ValueId, bool)> {
    match defs[v.0] { Some(Op::UnpackLo(x)) => Some((x, false)), Some(Op::UnpackHi(x)) => Some((x, true)), _ => None }
}

fn other_half(defs: &[Option<Op>], v: ValueId, x: ValueId, hi: bool) -> bool { half(defs, v) == Some((x, hi)) }

struct Rewrite { fresh: Vec<(Ty, Op)>, last: Op }

fn slot(i: usize) -> ValueId { ValueId(usize::MAX - i) }

fn lift(defs: &[Option<Op>], lo: ValueId, hi: ValueId) -> Option<Rewrite> {
    let bitwise = |v: ValueId| match defs[v.0] { Some(Op::Int(op @ (IntOp::And | IntOp::Or | IntOp::Xor), a, b)) => Some((op, a, b)), _ => None };
    ...
    if let Some((x, false)) = half(defs, lo) {
        if let Some((op, a, b)) = bitwise(hi) {
            let k = if other_half(defs, a, x, true) { b } else if other_half(defs, b, x, true) { a } else { return None };
            return Some(Rewrite { fresh: vec![(Ty::I32, identity(op)), (Ty::I64, Op::Pack64(slot(0), k))], last: Op::Int(op, x, slot(1)) });
        }
    }
    if let Some((x, true)) = half(defs, hi) {
        if let Some((op, a, b)) = bitwise(lo) {
            let k = if other_half(defs, a, x, false) { b } else if other_half(defs, b, x, false) { a } else { return None };
            return Some(Rewrite { fresh: vec![(Ty::I32, identity(op)), (Ty::I64, Op::Pack64(k, slot(0)))], last: Op::Int(op, x, slot(1)) });
        }
    }
    None
}
...
fn hoist(defs: &[Option<Op>], lo: ValueId, hi: ValueId) -> Option<Rewrite> {
    if let (Some((x, false)), Some(Op::Select(c, p, q))) = (half(defs, lo), defs[hi.0]) {
        if other_half(defs, q, x, true) { return Some(Rewrite { fresh: vec![(Ty::I64, Op::Pack64(lo, p))], last: Op::Select(c, slot(0), x) }); }
        if other_half(defs, p, x, true) { return Some(Rewrite { fresh: vec![(Ty::I64, Op::Pack64(lo, q))], last: Op::Select(c, x, slot(0)) }); }
    }
    if let (Some((x, true)), Some(Op::Select(c, p, q))) = (half(defs, hi), defs[lo.0]) {
        if other_half(defs, q, x, false) { return Some(Rewrite { fresh: vec![(Ty::I64, Op::Pack64(p, hi))], last: Op::Select(c, slot(0), x) }); }
        if other_half(defs, p, x, false) { return Some(Rewrite { fresh: vec![(Ty::I64, Op::Pack64(q, hi))], last: Op::Select(c, x, slot(0)) }); }
    }
    None
}
...
fn insert(defs: &[Option<Op>], lo: ValueId, hi: ValueId) -> Option<Rewrite> {
    ...
    if low.is_none() && high.is_none() { return None; }
    let mut fresh: Vec<(Ty, Op)> = Vec::new();
    let mut push = |ty: Ty, op: Op| -> ValueId { fresh.push((ty, op)); slot(fresh.len() - 1) };
    let low_word = match low {
        Some((x, false)) => { let mask = push(Ty::I64, Op::Const(Ty::I64, 0xffff_ffff)); push(Ty::I64, Op::Int(IntOp::And, x, mask)) }
        Some((x, true)) => { let shift = push(Ty::I64, Op::Const(Ty::I64, 32)); push(Ty::I64, Op::Int(IntOp::LShr, x, shift)) }
        None => push(Ty::I64, Op::Convert(Cvt::ZExt, Ty::I64, lo)),
    };
    let high_word = match high {
        Some((x, true)) => { let mask = push(Ty::I64, Op::Const(Ty::I64, 0xffff_ffff_0000_0000)); push(Ty::I64, Op::Int(IntOp::And, x, mask)) }
        Some((x, false)) => { let shift = push(Ty::I64, Op::Const(Ty::I64, 32)); push(Ty::I64, Op::Int(IntOp::Shl, x, shift)) }
        None => {
    ...
    Some(Rewrite { fresh, last: Op::Int(IntOp::Or, low_word, high_word) })
}
...
fn plan(f: &Func, defs: &[Option<Op>], ty: Ty, op: Op) -> Option<Rewrite> {
    match op {
        Op::Pack64(lo, hi) => match (half(defs, lo), half(defs, hi)) {
            ...
            _ => hoist(defs, lo, hi).or_else(|| lift(defs, lo, hi)).or_else(|| insert(defs, lo, hi)),
        },
        Op::Select(c, a, b) => match (half(defs, a), half(defs, b)) {
            (Some((x, false)), Some((y, false))) if f.types[x.0] == f.types[y.0] => Some(Rewrite { fresh: vec![(f.types[x.0], Op::Select(c, x, y))], last: Op::UnpackLo(slot(0)) }),
            (Some((x, true)), Some((y, true))) if f.types[x.0] == f.types[y.0] => Some(Rewrite { fresh: vec![(f.types[x.0], Op::Select(c, x, y))], last: Op::UnpackHi(slot(0)) }),
            _ => None,
        },
        Op::Convert(Cvt::ZExt, Ty::I64, a) if ty == Ty::I64 => match half(defs, a) {
            Some((x, false)) if f.types[x.0] == Ty::I64 => Some(Rewrite { fresh: vec![(Ty::I64, Op::Const(Ty::I64, 0xffff_ffff))], last: Op::Int(IntOp::And, x, slot(0)) }),
            Some((x, true)) if f.types[x.0] == Ty::I64 => Some(Rewrite { fresh: vec![(Ty::I64, Op::Const(Ty::I64, 32))], last: Op::Int(IntOp::LShr, x, slot(0)) }),
            _ => None,
        },
        _ => None,
    }
...
fn expand(f: &mut Func) -> usize {
    ...
    for (&id, block) in &f.blocks {
        for (index, inst) in block.insts.iter().enumerate() {
            let Inst::Core { ty, op, .. } = inst else { continue };
            if let Some(rewrite) = plan(f, &defs, *ty, *op) { plans.push((id, index, rewrite)); }
        }
    }
    let count = plans.len();
    for (id, index, rewrite) in plans.into_iter().rev() {
        let fresh: Vec<ValueId> = rewrite.fresh.iter().map(|(ty, _)| f.value(*ty)).collect();
        let place = |op: Op| op.map(|v| if v.0 > usize::MAX - fresh.len() - 1 { fresh[usize::MAX - v.0] } else { v });
        let insts: Vec<Inst> = rewrite.fresh.iter().enumerate().map(|(i, (ty, op))| Inst::Core { value: fresh[i], ty: *ty, op: place(*op) }).collect();
        let block = f.blocks.get_mut(&id).unwrap();
        if let Inst::Core { op, .. } = &mut block.insts[index] { *op = place(rewrite.last); }
    ...
    for block in f.blocks.values() {
        ...
        for inst in &block.insts {
            ...
            match seen.iter().find(|(t, o, _)| t == ty && o == op) {
                Some(&(_, _, previous)) => { map.insert(*value, previous); }
