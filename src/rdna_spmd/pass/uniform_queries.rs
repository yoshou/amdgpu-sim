...
mod tests {
    ...
    fn folded(shape: u8) -> Option<Op> {
        ...
        let reading = if shape == 2 {
            core(&mut f, &mut insts, Ty::I1, Op::Int(IntOp::And, exec, valid))
        } else {
            narrow
        };
        ...
        insts.push(Inst::Effect {
            ...
            inputs: vec![source, reading],
            outputs: vec![(answer, Ty::I32)],
        });
        f.blocks.insert(
            BlockId(0),
            Block {
        ...
        let inputs = [
            ...
            Parameter {
                source: ParameterSource::Vgpr(1),
                ty: Ty::I32,
            },
            Parameter {
                source: ParameterSource::MaskBit(106),
                ty: Ty::I1,
            },
            Parameter {
                source: ParameterSource::Sgpr(4),
                ty: Ty::I32,
            },
        ];
        if run(&mut f, &inputs) == 0 {
            return None;
        }
        let Term::Ret(returned) = &f.blocks[&BlockId(0)].term else {
    ...
    fn a_readfirstlane_folds_only_when_its_source_is_uniform_without_a_predicate() {
        assert!(
            folded(0).is_some(),
            "a wave-uniform dispatch input is the answer for every lane"
        );
        for (shape, why) in [
