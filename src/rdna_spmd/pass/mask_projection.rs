...
fn free_provenance(f: &Func) -> u64 {
    let used = f
        ...
        .filter_map(|inst| match inst {
            Inst::Effect { provenance, .. } => Some(*provenance & !(1u64 << 63) & !SCHEDULED),
            _ => None,
        })
        .max()
        .unwrap_or(0);
...
fn incoming<'a>(f: &'a Func, block: BlockId, index: usize) -> impl Iterator<Item = ValueId> + 'a {
    f.blocks
        ...
        .filter(move |e| e.dst == block)
        .map(move |e| e.args[index])
}

impl Mask {
    fn of(&self, value: ValueId) -> usize {
        self.repr[value.0].0
    }
    ...
    fn new(f: &Func, constants: &[Option<u64>]) -> Self {
        ...
        for block in f.blocks.values() {
            for inst in &block.insts {
                if let Inst::Core {
                    value,
                    op: Op::Convert(Cvt::Bitcast, ty, source),
                    ..
                } = inst
                {
                    if ty.bits() == f.types[source.0].bits() {
                        repr[value.0] = repr[source.0];
                    }
                }
            }
        }
        ...
        for (&id, block) in &f.blocks {
            for (index, &(value, _)) in block.params.iter().enumerate() {
                m.parameter[value.0] = Some((id, index));
            }
            for inst in &block.insts {
                match inst {
                    Inst::Effect {
                        ...
                        outputs,
                        ..
                    } => m.ballot[m.repr[outputs[0].0 .0].0] = Some(inputs[0]),
                    Inst::Packet {
                        op: PacketOp::Ballot,
                        input,
                        output,
                    } => m.ballot[m.repr[output.0].0] = Some(*input),
                    _ => {}
                }
            }
        }
            ...
            for inst in &block.insts {
                ...
                match core {
                    ...
                    Some(Op::Int(IntOp::LShr, word, lane))
                        if matches!(m.defs[lane.0], Some(Op::Env(Env::LaneId))) =>
                    {
                        m.uses[m.repr[word.0].0].push(Use::Extract(result(inst).unwrap()));
                        m.uses[m.repr[lane.0].0].push(Use::Other);
                    }
                    Some(Op::Cmp(IntPred::Ne | IntPred::Eq, a, b))
                        if m.constants[a.0] == Some(0) || m.constants[b.0] == Some(0) =>
                    {
                        let (word, zero) = if m.constants[b.0] == Some(0) {
                            (a, b)
                        } else {
                            (b, a)
                        };
            ...
            match &block.term {
                Term::Br(e) => {
                    for (i, &v) in e.args.iter().enumerate() {
                        m.uses[m.repr[v.0].0].push(Use::Argument(e.dst, i));
                    }
                }
                ...
                Term::Ret(args) => {
                    for &v in args {
                        m.uses[m.repr[v.0].0].push(Use::Other);
                    }
                }
            }
        }
    ...
    fn settle(&mut self, f: &Func) {
        ...
        while changed {
            ...
            for (&id, block) in &f.blocks {
                for inst in &block.insts {
                    let Inst::Core { value, op, .. } = inst else {
                        continue;
                    };
                    let residue = match *op {
                        Op::Env(Env::OutsideLanes) => true,
                        Op::Int(_, a, b) | Op::Select(_, a, b) | Op::Cmp(_, a, b) => {
                            self.residue[a.0] || self.residue[b.0]
                        }
                        Op::Convert(_, _, a) => self.residue[a.0],
                        _ => false,
                    };
                    if residue && !self.residue[value.0] {
                        self.residue[value.0] = true;
                        changed = true;
                    }
                }
                for (index, &(value, _)) in block.params.iter().enumerate() {
                    if self.residue[value.0] || id == f.entry {
                        continue;
                    }
                    if incoming(f, id, index).any(|arg| self.residue[arg.0]) {
                        self.residue[value.0] = true;
                        changed = true;
                    }
                }
        ...
        for (v, ty) in f.types.iter().enumerate() {
            if self.repr[v] != ValueId(v) || self.residue[v] {
                continue;
            }
            if *ty != Ty::I32
                && self.ballot[v].is_none()
                && !matches!(self.defs[v], Some(Op::Select(..)))
            {
                continue;
            }
            self.word[v] = self.ballot[v].is_some()
                ...
                || matches!(
                    self.defs[v],
                    Some(Op::Int(IntOp::And | IntOp::Or | IntOp::Xor, ..)) | Some(Op::Select(..))
                )
        ...
        while changed {
            ...
            for v in 0..f.types.len() {
                if self.word[v] && !self.holds(ValueId(v), f) {
                    self.word[v] = false;
                    changed = true;
                }
            }
...
impl Emit {
    ...
    fn operand(&mut self, value: ValueId, f: &Func) -> ValueId {
        ...
        let lane = match self.lane {
            Some(lane) => lane,
            None => {
                let lane = self.push(Ty::I32, Op::Env(Env::LaneId));
                self.lane = Some(lane);
                lane
            }
        };
    ...
    fn beyond(&mut self, value: ValueId, f: &Func) -> ValueId {
        if let Some(out) = self.out[value.0] {
            return out;
        }
        if let Some(&out) = self.masked.get(&value) {
            return out;
        }
        let value = self.as_word(value, f);
        let lanes = match self.lanes {
            Some(lanes) => lanes,
            None => {
                let lanes = self.push(Ty::I32, Op::Env(Env::OutsideLanes));
...
pub(crate) fn run(f: &mut Func, constants: &[Option<u64>]) -> usize {
    ...
    if mask.ballot.iter().all(Option::is_none) {
        return 0;
    }
    mask.settle(f);
    let mut required = vec![false; f.types.len()];
    let mut beyond = vec![false; f.types.len()];
    let sites = |kinds: &[bool]| {
        (0..f.types.len())
            .map(ValueId)
            .filter(|v| {
                mask.repr[v.0] == *v
                    && mask.word[v.0]
                    && mask.uses[v.0].iter().any(|u| match u {
                        Use::Extract(_) => kinds[0],
                        Use::Nonzero => kinds[1],
                        _ => false,
                    })
            })
    ...
    for (mark, mut pending, through) in [
        ...
        while let Some(value) = pending.pop() {
            ...
            if let Some((block, index)) = mask.parameter[v] {
                for arg in incoming(f, block, index).collect::<Vec<_>>() {
                    let arg = mask.repr[arg.0];
                    if through[arg.0] {
                        pending.push(arg);
                    }
                }
    ...
    let mut emit = Emit {
        ...
        masked: BTreeMap::new(),
        lanes: None,
        bit: vec![None; f.types.len()],
        out: vec![None; f.types.len()],
        provenance: free_provenance(f),
    };
        ...
        for (index, inst) in block.insts.iter().enumerate() {
            ...
            if required[value.0] {
                let bit = match (
                    ...
                    (None, None, Some(Op::Int(op, a, b))) => {
                        let (a, b) = (
                            emit.operand(mask.repr[a.0], f),
                            emit.operand(mask.repr[b.0], f),
                        );
            ...
            if beyond[value.0] {
                let out = match (mask.ballot[value.0], mask.defs[value.0]) {
                    ...
                    (None, Some(Op::Int(op @ (IntOp::And | IntOp::Or | IntOp::Xor), a, b))) => {
                        let (a, b) = (
                            emit.beyond(mask.repr[a.0], f),
                            emit.beyond(mask.repr[b.0], f),
                        );
            ...
            match mask.defs[value.0] {
                Some(Op::Convert(Cvt::Trunc, Ty::I1, shifted)) => {
                    let Some(Op::Int(IntOp::LShr, word, _)) = mask.defs[shifted.0] else {
                        continue;
                    };
                    if !required[word.0] || !mask.truncated[shifted.0] {
                        continue;
                    }
                    ...
                    if mask.constants[a.0] == Some(0) || mask.constants[b.0] == Some(0) =>
                {
                    ...
                    emit.provenance += 1;
                    assert!(
                        provenance & SCHEDULED == 0,
                        "synthesized query provenance overflowed"
                    );
                    emit.pending.push(Inst::Effect {
                        provenance: (1u64 << 63) | provenance,
    ...
    for (&id, indices) in &parameters {
        ...
        for &(index, outside) in indices {
            ...
            let (companion, ty) = if outside {
                (emit.out[value.0], Ty::I32)
            } else {
                (emit.bit[value.0], Ty::I1)
            };
    ...
    for id in f.blocks.keys().copied().collect::<Vec<_>>() {
        ...
        let carried: Vec<(usize, Vec<(ValueId, bool)>)> = f.blocks[&id]
            .term
            .edges()
            .enumerate()
            .filter_map(|(slot, edge)| {
                parameters.get(&edge.dst).map(|indices| {
                    (
                        slot,
                        indices
                            .iter()
                            .map(|&(i, outside)| (edge.args[i], outside))
                            .collect(),
                    )
                })
            })
            .collect();
        let appended: Vec<(usize, Vec<ValueId>)> = carried
            .into_iter()
            .map(|(slot, args)| {
                (
                    slot,
        ...
        emit.flush(id, end);
        let block = f.blocks.get_mut(&id).unwrap();
        for (slot, bits) in appended {
            block.term.edges_mut().nth(slot).unwrap().args.extend(bits);
        }
    }
    ...
    for (block, sites) in emit.inserted {
        ...
        for (index, insts) in sites.into_iter().rev() {
            b.insts.splice(index..index, insts);
        }
    }
    for block in f.blocks.values_mut() {
        block
            .insts
            .retain(|inst| !matches!(inst, Inst::Core { value, .. } if dropped.contains(value)));
    }
