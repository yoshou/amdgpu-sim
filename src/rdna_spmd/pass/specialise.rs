...
pub(crate) fn candidates(f: &Func, masks: &Masks, exec_index: usize) -> Vec<BlockId> {
    ...
    for (&id, block) in &f.blocks {
        ...
        let k = entry.params[..exec_index]
            ...
            .filter(|p| p.1 == Ty::I1)
            .count();
        let Some(&(exec, _)) = block.params.iter().filter(|p| p.1 == Ty::I1).nth(k) else {
            continue;
        };
        if masks.full[exec.0] {
            continue;
        }
...
pub(crate) fn run(f: &mut Func, blocks: &[BlockId], exec_index: usize) -> usize {
    ...
    for &id in blocks {
        ...
        let masked = BlockId(next_id);
        next_id += 1;
        let all = BlockId(next_id);
        next_id += 1;
        let mut map: BTreeMap<ValueId, ValueId> = BTreeMap::new();
        let fresh = |f: &mut Func, v: ValueId, map: &mut BTreeMap<ValueId, ValueId>| {
            let n = f.value(f.types[v.0]);
            map.insert(v, n);
            n
        };
        let mut clone = Block {
        ...
        for &(p, ty) in &original.params {
            let n = fresh(f, p, &mut map);
            clone.params.push((n, ty));
        }
        for inst in &original.insts {
            let cloned = match inst {
                Inst::Core { value, ty, op } => {
                    let n = fresh(f, *value, &mut map);
                    Inst::Core {
                        value: n,
                        ty: *ty,
                        op: *op,
                ...
                Inst::Target {
                    ...
                    let outputs = outputs
                        .iter()
                        .map(|&(v, ty)| (fresh(f, v, &mut map), ty))
                        .collect();
                    Inst::Target {
                        provenance: provenance.map(|p| p + (1 << 40)),
                        op: *op,
                        args: *args,
                        outputs,
                    }
        ...
        for &(p, ty) in &original.params {
            let n = f.value(ty);
            guard_map.insert(p, n);
            guard.params.push((n, ty));
        }
        ...
        guard.insts.push(Inst::Core {
            value: inactive,
            ty: Ty::I1,
            op: Op::Int(IntOp::And, flipped, valid),
        });
        guard.insts.push(Inst::Packet {
            op: PacketOp::Any,
            input: inactive,
            output: any,
        });
        let args: Vec<ValueId> = guard.params.iter().map(|p| p.0).collect();
        guard.term = Term::CondBr {
            cond: any,
            yes: Edge {
                dst: masked,
                args: args.clone(),
            },
            no: Edge { dst: all, args },
        };
...
fn original_exec(block: &Block, f: &Func, entry: BlockId, exec_index: usize) -> ValueId {
    ...
    let k = entry_block.params[..exec_index]
        .iter()
        .filter(|p| p.1 == Ty::I1)
        .count();
    block
