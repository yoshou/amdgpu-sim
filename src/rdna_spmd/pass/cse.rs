...
fn run(f: &mut Func) -> usize {
    ...
    for block in f.blocks.values_mut() {
        ...
        block.insts.retain(|inst| {
            ...
            if !matches!(op, WaveOp::Any | WaveOp::Ballot | WaveOp::ReadFirstLane) {
                return true;
            }
            ...
            if let Some((_, _, previous)) = seen
                .iter()
                .find(|(kind, args, _)| kind == op && args == inputs)
            {
                renames.insert(outputs[0].0, *previous);
                false
            } else {
                seen.push((*op, inputs.clone(), outputs[0].0));
                true
            }
        });
    }
...
mod tests {
    ...
    fn reuses_answers_without_conflating_sources_masks_or_blocks() {
        ...
        let next_source = f.value(Ty::I32);
        let next_exec = f.value(Ty::I1);
        ...
        f.blocks.insert(
            ...
            Block {
                ...
                insts: vec![Inst::Effect {
                    ...
                    inputs: vec![next_source, next_exec],
                    outputs: vec![(next, Ty::I32)],
                }],
        ...
        f.check(&registry).unwrap();
        let kept: BTreeMap<_, _> = f.blocks[&BlockId(0)]
            .insts
            .iter()
        ...
        let Term::Br(edge) = &f.blocks[&BlockId(0)].term else {
            unreachable!()
        };
        assert_eq!(
            &edge.args[2..],
            &[
                kept[&0], kept[&0], kept[&2], kept[&2], kept[&4], kept[&4], kept[&6], kept[&7],
                kept[&8]
            ]
        );
        assert!(matches!(
            f.blocks[&BlockId(1)].insts[0],
            Inst::Effect { .. }
        ));
    ...
    fn a_reused_ballot_crossing_a_block_edge_still_projects_to_its_lane_bit() {
        ...
        let nonzero = f.value(Ty::I1);
        let carried = f.value(Ty::I32);
        let previous = f.value(Ty::I1);
        let next_zero = f.value(Ty::I32);
        let next_nonzero = f.value(Ty::I1);
        f.blocks.insert(
            BlockId(0),
            Block {
                params: vec![(predicate, Ty::I1)],
                insts: vec![
                    Inst::Effect {
                        provenance: 0,
                    ...
                    Inst::Core {
                        value: nonzero,
                        ty: Ty::I1,
                        op: Op::Cmp(IntPred::Ne, first, zero),
                    },
                ],
                term: Term::Br(Edge {
                    dst: BlockId(1),
                    args: vec![second, nonzero],
                }),
            },
        );
        f.blocks.insert(
            BlockId(1),
            Block {
                params: vec![(carried, Ty::I32), (previous, Ty::I1)],
                insts: vec![
                    Inst::Core {
                        value: next_zero,
                        ty: Ty::I32,
                    ...
                    Inst::Core {
                        value: next_nonzero,
                        ty: Ty::I1,
                        op: Op::Cmp(IntPred::Ne, carried, next_zero),
                    },
                ],
        ...
        let registry = crate::rdna_spmd::targets::rdna4::registry();
        for _ in 0..4 {
            let constants = Analyses::new(super::super::super::analysis::Context::new(
                &registry,
                &[],
                0,
                16,
            ))
            .get::<super::super::super::analysis::Constants>(&f);
