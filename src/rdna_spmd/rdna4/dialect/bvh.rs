...
pub fn lower(e: &Emitter, a: &[Value]) -> Vec<Value> {
    ...
    let params = [
        ptr, ptr, ptr, ptr, i32t, i32t, i64t, f32t, f32t, f32t, f32t, f32t, f32t, f32t, f32t, f32t,
        f32t,
    ];
    let args = [
        scratch_ptr(0),
...
pub fn lower8(e: &Emitter, a: &[Value]) -> Vec<Value> {
    ...
    let params = [
        ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, i32t, i32t, i64t, f32t, i32t, f32t, f32t,
        f32t, f32t, f32t, f32t, i32t,
    ];
    let func = ir.current_function();
    for l in 0..lanes {
...
impl Native {
    ...
    fn splat(&self, v: Value, vty: Type) -> Value {
        self.ir.splat(v, vty.vector_size())
    }
    fn vci32(&self, v: u32) -> Value {
        self.splat(self.ci32(v), self.vi32)
    }
    fn zext64s(&self, v: Value) -> Value {
        self.ir.zext(v, self.ir.i64())
    }
    fn vf32_bits(&self, v: Value) -> Value {
        self.ir.bitcast(v, self.vi32)
    }
    fn vf32_of(&self, v: Value) -> Value {
        self.ir.bitcast(v, self.vf32)
    }
    ...
    fn vfadd(&self, a: Value, b: Value) -> Value {
        self.ir.fadd(a, b)
    }
    fn vfmul(&self, a: Value, b: Value) -> Value {
        self.ir.fmul(a, b)
    }
    fn vfsub(&self, a: Value, b: Value) -> Value {
        self.ir.fsub(a, b)
    }
    ...
    fn node_address(&self, bvh_base: Value, rep: Value) -> Value {
        let offset = self
            .ir
            .shl(self.ir.and(rep, self.ci64(!0x7u64)), self.ci64(3));
    ...
    fn box_path(
        ...
        for c in 0..4u64 {
            ...
            for axis in 0..3u64 {
                ...
                let f = self.vfmul(
                    self.vfsub(bhi, ray.origin[axis as usize]),
                    ray.inv[axis as usize],
                );
                let g = self.vfmul(
                    self.vfsub(blo, ray.origin[axis as usize]),
                    ray.inv[axis as usize],
                );
                for v in [f, g] {
            ...
            let t1 = self.vminnum_raw(
                hi3[0],
                self.vminnum_raw(hi3[1], self.vminnum_raw(hi3[2], ray.extent)),
            );
    ...
    fn triangle_path(
        ...
        let t0v: Vec<Value> = (0..3)
            ...
            .collect();
        let t1v: Vec<Value> = (0..3)
            .map(|k| self.splat(pick(3, 1, k), self.vf32))
            .collect();
        let t2v: Vec<Value> = (0..3)
            .map(|k| self.splat(pick(2, 2, k), self.vf32))
            .collect();
        ...
        let reject_pos = and(
            ...
            or(
                or(
                    or(
                        fc(FloatPred::Olt, b_y, zero),
                        fc(FloatPred::Ogt, b_y, denom),
                    ),
                    or(
                        fc(FloatPred::Olt, b_z, zero),
                        fc(FloatPred::Ogt, byz, denom),
                    ),
                ),
        ...
        let reject_neg = and(
            ...
            or(
                or(
                    or(
                        fc(FloatPred::Ogt, b_y, zero),
                        fc(FloatPred::Olt, b_y, denom),
                    ),
                    or(
                        fc(FloatPred::Ogt, b_z, zero),
                        fc(FloatPred::Olt, byz, denom),
                    ),
                ),
        ...
        let tri_bits = [
            self.vf32_bits(tri_res[0]),
            self.vf32_bits(tri_res[1]),
            self.vf32_bits(tri_res[2]),
            self.vf32_bits(tri_res[3]),
        ];
        ir.br(join);
    ...
    fn general_path(
        ...
        let ir = self.ir;
        let (bvh_packet, bvh_packet_ty) = self
            .bvh_packet
            .expect("packet lowering requires the ray packet frame");
        ...
        let step = self
            .w
            .min(crate::rdna_translator::bvh::BVH_RAY_PACKET_LANES as u32);
        ...
        while off < self.w {
            ...
            for (f, value) in inputs.iter().copied().enumerate() {
                ir.store(slice(value, off, len), field_ptr(f as u32))
                    .set_alignment(if f == 0 { 8 } else { 4 });
            }
            ir.call_named(
            ...
            chunks.push(
                (0..4)
                    .map(|k| {
                        ir.load(ir.i32().vector(len), field_ptr(11 + k))
                            .set_alignment(4)
                    })
                    .collect(),
            );
