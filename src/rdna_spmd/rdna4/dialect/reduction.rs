...
pub fn lower(e: &Emitter, args: &[Value]) -> Value {
    ...
    let read = |index: Value| {
        ...
        if index.is_vector() {
            ...
            let call = e.call(
                &format!("llvm.masked.gather.v{w}i64.v{w}p0"),
                Ty::I64,
            ...
            ir.load(e.ty(Ty::I64), ptr).set_alignment(8)
        }
    };
    let lo = read(word);
    let hi = read(ir.add(word, k(1)));
    let fraction = e.call(
        &format!("llvm.fshr.{}", e.suffix(Ty::I64)),
        Ty::I64,
        &[hi, lo, bit],
    );
