...
pub fn lift(
        ...
        for inst in &block.body {
            ...
            regs.extend(
                super::access::f64_pairs(inst)
                    .into_iter()
                    .flat_map(|r| [Word::Vgpr(r), Word::Vgpr(r + 1)]),
            );
            regs.extend(
            ...
            regs.extend(
                super::access::vgpr_reads(inst)
                    .into_iter()
                    .chain(super::access::div_reads(inst))
                    .map(Word::Vgpr),
            );
        }
    }
    ...
    for (&pc, source) in &program.blocks {
        ...
        block.term = match source.term {
            ...
            Terminator::Jump(pc)
            | Terminator::Barrier { resume: pc }
            | Terminator::Yield { resume: pc, .. } => Term::Br(edge(pc)),
            Terminator::Branch {
                cond,
                taken,
                fallthrough,
    ...
    let parameter_inputs: Vec<_> = regs
        .iter()
        .map(|word| Parameter {
            ty: word.ty(),
            source: match *word {
...
fn invalidate(views: &mut BTreeMap<(Word, Ty, bool), ValueId>, writes: &[Word]) {
    views.retain(|&(r, t, _), _| {
        !(0..t.bits().div_ceil(32))
            .filter_map(|k| r.offset(k))
...
fn alu(
    ...
    let previous: Vec<_> = outputs
        ...
        .map(|output| match *output {
            Output::Vgpr(reg, ty) => {
                let input = Input {
                    source: InputSource::Operand(SourceOperand::VectorRegister(reg as u8)),
                    ty,
                };
    ...
    for inst in &expr.expr().insts {
        match inst {
            ...
            ExprInst::Target { op, args, outputs } => {
                ...
                let id = if effect == crate::rdna_spmd::dialect::Effect::Pure {
                    None
                } else {
                    let id = *provenance;
                    *provenance += 1;
    ...
    let results: Vec<_> = outputs
        .iter()
        .copied()
        .zip(expr.expr().results.iter().map(|id| values[id.0]))
        .collect();
    let defined = super::regs::define(
        f,
        &mut block.insts,
        words,
        &results,
    ...
    for (&output, &(stored, _)) in outputs.iter().zip(&stored) {
        match output {
            Output::Vgpr(reg, ty) => {
                views.insert((Word::Vgpr(reg), ty, false), stored);
            }
            Output::Scalar(reg, ty) => {
                if let Some(reg) = Word::scalar(reg).filter(|r| r.ordinary_span(ty)) {
