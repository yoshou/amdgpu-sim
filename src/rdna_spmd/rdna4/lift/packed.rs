...
pub fn instruction(inst: &InstFormat, registry: &DialectRegistry) -> Option<Lowering> {
    ...
    if let Some(dot) = dot_integer(i, registry) {
        return Some(dot);
    }
    if let Some(float) = packed_float(i, registry) {
        return Some(float);
    }
    if let Some(mixed) = mixed_float(i, registry) {
        return Some(mixed);
    }
    if let Some(dot) = dot_float(i, registry) {
        return Some(dot);
    }
    ...
    let mut b = Builder::new(
        ...
        [&i.src0, &i.src1, &i.src2]
            .iter()
            .take(arity)
            .map(|s| input((*s).clone(), Ty::I32))
            .collect(),
    );
    let mut halves = vec![];
    for (select, neg) in [
        (i.opsel, i.neg),
        (i.opsel_hi | (i.opsel_hi2 << 2), i.neg_hi),
    ] {
        ...
        for source in 0..arity {
            ...
            let selected = if (select >> source) & 1 != 0 {
                b.int(IntOp::LShr, raw, shift)
            } else {
                raw
            };
        ...
        let mut result = match i.op {
            I::V_PK_MIN_I16 | I::V_PK_MIN_U16 | I::V_PK_MAX_I16 | I::V_PK_MAX_U16 => {
                ...
                let predicate = match (minimum, signed) {
                    (true, true) => IntPred::Slt,
                    (true, false) => IntPred::Ult,
                    (false, true) => IntPred::Sgt,
                    (false, false) => IntPred::Ugt,
                };
...
fn dot_integer(
    ...
    let (bits, signed) = match i.op {
        I::V_DOT4_I32_IU8 => (8, true),
        I::V_DOT4_U32_U8 => (8, false),
        I::V_DOT8_I32_IU4 => (4, true),
        I::V_DOT8_U32_U4 => (4, false),
        _ => return None,
    };
    let mut b = Builder::new(
        registry,
    ...
    let mut sum = b.push(
        Ty::I64,
        Op::Convert(
            if signed { Cvt::SExt } else { Cvt::ZExt },
            Ty::I64,
            ValueId(2),
        ),
    );
    for select in [i.opsel, i.opsel_hi | i.opsel_hi2 << 2] {
        for component in 0..16 / bits {
            ...
            for source in 0..2 {
                let shift = b.k(
                    Ty::I32,
                    ((select >> source & 1) as u64) * 16 + component * bits,
                );
                ...
                values.push(b.push(
                    Ty::I64,
                    Op::Convert(if negative { Cvt::SExt } else { Cvt::ZExt }, Ty::I64, value),
                ));
            }
    ...
    if i.cm != 0 {
        ...
        let above = b.push(
            Ty::I1,
            Op::Cmp(if signed { IntPred::Sgt } else { IntPred::Ugt }, sum, high),
        );
...
fn packed_float(
    ...
    let mut b = Builder::new(
        ...
        [i.src0, i.src1, i.src2]
            ...
            .copied()
            .take(arity)
            .map(|s| input(half::source(s), Ty::I32))
            .collect(),
    );
    let widen =
    ...
    for (select, neg) in [(i.opsel, i.neg), (i.opsel_hi | i.opsel_hi2 << 2, i.neg_hi)] {
        ...
        for index in 0..arity {
            ...
            if select >> index & 1 != 0 {
                let shift = b.k(Ty::I32, 16);
                value = b.int(IntOp::LShr, value, shift);
            }
        ...
        let mut value = if wide {
            let args = values
                .iter()
                .map(|&v| b.push(Ty::F64, Op::Convert(Cvt::FloatResizeRte, Ty::F64, v)))
                .collect::<Vec<_>>();
            b.push(Ty::F64, Op::Fma(args[0], args[1], args[2]))
        } else {
            let op = match i.op {
                I::V_PK_ADD_F16 => FloatOp::Add,
                I::V_PK_MUL_F16 => FloatOp::Mul,
                I::V_PK_MIN_NUM_F16 | I::V_PK_MINIMUM_F16 => FloatOp::MinNum,
                _ => FloatOp::MaxNum,
            };
        ...
        results.push(if wide {
            ...
            b.target_one(narrow, Arguments::Unary(value))
        });
    }
    let shift = b.k(Ty::I32, 16);
...
fn mixed_float(
    ...
    let (partial, high) = match i.op {
        I::V_FMA_MIX_F32 => (false, false),
        I::V_FMA_MIXLO_F16 => (true, false),
        I::V_FMA_MIXHI_F16 => (true, true),
        _ => return None,
    };
    let precision = i.opsel_hi | i.opsel_hi2 << 2;
    let mut inputs = [i.src0, i.src1, i.src2]
        .iter()
        .copied()
        .enumerate()
        .map(|(index, s)| {
        ...
        .collect::<Vec<_>>();
    if partial {
        inputs.push(input(SourceOperand::VectorRegister(i.vdst), Ty::I32));
    }
    ...
    for index in 0..3 {
        let mut value = ValueId(index);
        if precision >> index & 1 != 0 {
            if i.opsel >> index & 1 != 0 {
                let shift = b.k(Ty::I32, 16);
                value = b.int(IntOp::LShr, value, shift);
            }
            value = b.target_one(widen, Arguments::Unary(value));
        } else {
            value = b.push(Ty::F32, Op::Convert(Cvt::Bitcast, Ty::F32, value));
        }
    ...
    Some(b.finish(
        Output::Vgpr(i.vdst as u32, if partial { Ty::I32 } else { Ty::F32 }),
        value,
    ))
}
    ...
    for (select, neg) in [(i.opsel, i.neg), (i.opsel_hi | i.opsel_hi2 << 2, i.neg_hi)] {
        ...
        for index in 0..2 {
            ...
            value = if bf16 {
                ...
                b.target_one(widen, Arguments::Unary(value))
            };
