...
pub(super) fn query(f: &mut Func, insts: &mut Vec<Inst>, op: WaveOp, bit: ValueId) -> ValueId {
    ...
    insts.push(Inst::Effect {
        provenance: (1u64 << 63) | value.0 as u64,
        op: EffectOp::Wave(op),
        inputs: vec![bit],
...
impl Operands {
    pub fn read(
        ...
        if let Some(bits) = input.constant_bits() {
            ...
            self.core.push(Inst::Core {
                value,
                ty: input.ty,
                op: Op::Const(input.ty, bits),
            });
            return value;
        }
        ...
        if matches!(
            input.source,
            InputSource::Operand(SourceOperand::PrivateBase)
        ) {
            let raw = core(f, &mut self.core, Ty::I64, Op::Env(Env::ScratchBase));
            let value = if input.ty == Ty::I64 {
                raw
            } else {
                let word = core(
                    f,
                    &mut self.core,
                    Ty::I32,
                    Op::Convert(Cvt::Trunc, Ty::I32, raw),
                );
                match input.ty {
                    Ty::I32 => word,
                    ...
                    Ty::F32 => core(
                        ...
                        &mut self.core,
                        Ty::F32,
                        Op::Convert(Cvt::Bitcast, Ty::F32, word),
                    ),
                ...
                if (0..count).any(|k| matches!(r as u32 + k, 106 | 124 | 126)) {
                    ...
                    return if input.ty.integer() {
                        ...
                        core(
                            ...
                            &mut self.core,
                            input.ty,
                            Op::Convert(Cvt::Bitcast, input.ty, raw),
                        )
                    };
                }
            }
        ...
        let word = match &input.source {
            InputSource::Operand(source) => {
                Word::source(source).filter(|r| r.ordinary_span(input.ty))
            }
            _ => None,
        };
        ...
        if let Some(word) = word {
            if input.ty.bits() == 32 {
                ...
                self.core.push(Inst::Core {
                    value,
                    ty: Ty::F32,
                    op: Op::Convert(Cvt::Bitcast, Ty::F32, raw),
                });
            ...
            if input.ty.bits() == 64 {
                ...
                let value = if input.ty == Ty::F64 {
                    ...
                    self.core.push(Inst::Core {
                        value,
                        ty: Ty::F64,
                        op: Op::Convert(Cvt::Bitcast, Ty::F64, pair),
                    });
                    value
                } else {
                    pair
    ...
    match lowering {
        Lowering::TypedAlu {
            ...
            for output in outputs {
                match *output {
                    Output::Vgpr(r, t) => {
                        for k in 0..t.bits().div_ceil(32) {
                            io.writes.add_vgpr(r + k);
                        }
                    }
                    Output::Scalar(r, t) => {
                        for k in 0..t.bits().div_ceil(32) {
        ...
        Lowering::Memory(m) => {
            ...
            match m.address {
                ...
                Address::Scalar {
                    ...
                    io.reads.add_sgpr(base);
                    io.reads.add_sgpr(base + 1);
                    if let Some(r) = scalar_offset {
                        io.reads.add_sgpr(r);
                    }
                }
                Address::Scratch {
            ...
            if m.returns {
                for r in m.dest..m.dest + m.words {
                    if m.scalar() {
                        io.writes.add_sgpr(r);
                    } else {
                        io.writes.add_vgpr(r);
                    }
                }
            }
        }
...
pub(super) fn define(
    ...
    for &(output, value) in results {
        if matches!(output, Output::Scc) {
            *scc = if scalar {
                value
            } else {
                query(f, insts, WaveOp::Any, value)
            };
        }
    }
    let exec = core(
    ...
    for &r in writes {
        words.insert(
            ...
            *word_defs
                .get(&r)
                .expect("typed output lacks its architectural SSA definition"),
        );
    }
    Definitions { stored }
}
...
pub(super) fn branch(
    ...
    let query = if matches!(cond, Cond::Scc0 | Cond::Scc1) {
        input_value
    } else {
    ...
    let result = if matches!(cond, Cond::Scc0 | Cond::ExecZ | Cond::VccZ) {
        ...
        insts.push(Inst::Core {
            value: result,
            ty: Ty::I1,
            op: Op::Cmp(IntPred::Eq, query, zero),
        });
        result
    } else {
        query
