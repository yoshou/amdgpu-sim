...
pub fn instruction(inst: &InstFormat, registry: &DialectRegistry) -> Option<Lowering> {
    if let InstFormat::VOP3SD(i) = inst {
        ...
        let mut b = Builder::new(
            registry,
            vec![input(i.src0, ty), input(i.src1, ty), input(i.src2, ty)],
        );
        ...
        if i.sdst != 124 {
            outputs.push((Output::Mask(i.sdst as u32), values[1]));
        }
    ...
    if fmas {
        inputs.push(input(SourceOperand::ScalarRegister(106), Ty::I1));
    }
    ...
    let args = if fmas {
        Arguments::Quaternary([a, c, d, ValueId(3)])
    } else {
        Arguments::Ternary([a, c, d])
    };
