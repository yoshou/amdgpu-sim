...
pub fn vgpr_reads(inst: &InstFormat) -> Vec<u32> {
    ...
    if let Some(memory) = super::memory::instruction(inst) {
        return memory.reads();
    }
    if let Some(wave) = super::wave::instruction(inst) {
        return wave.io().reads.vgprs().collect();
    }
    let mut r = Vec::new();
    match inst {
        ...
        InstFormat::VOP3P(i) => {
            ...
            src_vgpr(&i.src1, &mut r);
            src_vgpr(&i.src2, &mut r);
            if matches!(i.op, I::V_FMA_MIXLO_F16) {
                r.push(i.vdst as u32);
            }
        }
        _ => {}
...
fn op_is_pair(op: I) -> bool {
    ...
    (s.contains("F64") && !matches!(op, I::V_CVT_F64_U32 | I::V_CVT_F64_I32 | I::V_CVT_I32_F64))
        || matches!(
            op,
            I::V_MAD_CO_U64_U32 | I::V_LSHLREV_B64 | I::V_ASHR_I64 | I::V_ASHRREV_I64
        )
}
...
fn push_v(r: &mut Vec<u32>, idx: u8, pair: bool) {
    r.push(idx as u32);
    if pair {
        r.push(idx as u32 + 1);
    }
...
pub fn div_reads(inst: &InstFormat) -> Vec<u32> {
    ...
    match inst {
        ...
        InstFormat::VOP2(i) => {
            let p = op_is_pair(i.op);
            push_src(&mut r, &i.src0, p);
            push_v(&mut r, i.vsrc1, p);
        }
        InstFormat::VOP3(i) => {
            let p = op_is_pair(i.op);
            push_src(&mut r, &i.src0, p);
            push_src(&mut r, &i.src1, p);
            push_src(&mut r, &i.src2, p);
        }
        InstFormat::VOP3SD(i) => {
            let p = op_is_pair(i.op);
            push_src(&mut r, &i.src0, p);
        ...
        InstFormat::VOPD(i) => {
            push_src(&mut r, &i.src0x, false);
            push_v(&mut r, i.vsrc1x, false);
            push_src(&mut r, &i.src0y, false);
...
pub fn f64_pairs(inst: &InstFormat) -> Vec<u32> {
    ...
    let mut source = |op, src: &SourceOperand| {
        if reads(op) {
            if let SourceOperand::VectorRegister(r) = src {
                pairs.push(*r as u32);
            }
        }
    };
    let (op, dest) = match inst {
        InstFormat::VOP1(i) => {
            source(i.op, &i.src0);
            (Some(i.op), Some(i.vdst as u32))
        }
        InstFormat::VOP2(i) => {
            source(i.op, &i.src0);
            source(i.op, &SourceOperand::VectorRegister(i.vsrc1));
            (Some(i.op), Some(i.vdst as u32))
        }
        InstFormat::VOPC(i) => {
            source(i.op, &i.src0);
        ...
        InstFormat::VOP3(i) => {
            ...
            (
                Some(i.op),
                (!format!("{:?}", i.op).contains("V_CMP")).then_some(i.vdst as u32),
            )
        }
        InstFormat::VOP3SD(i) => {
            source(i.op, &i.src0);
    ...
    if let (Some(op), Some(dst)) = (op, dest) {
        if reads(op) || matches!(op, I::V_CVT_F64_U32 | I::V_CVT_F64_I32) {
            pairs.push(dst);
        }
    }
    pairs
...
pub fn math_reads(inst: &InstFormat) -> Vec<u32> {
    ...
    match inst {
        ...
        InstFormat::VOPD(i) => {
            ...
            if let SourceOperand::VectorRegister(x) = i.src0y {
                r.push(x as u32);
            }
            r.push(i.vsrc1x as u32);
            r.push(i.vsrc1y as u32);
